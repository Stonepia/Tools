...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/float32/2e/c2e5dmoe35bbw6qh6uhaclytasi27pgpeesjym633jhzyft2ew3s.py
...
@pointwise(size_hints=[33554432], filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_copy_new_zeros_select_scatter_slice_slice_scatter_5', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2), equal_to_1=())]})
@triton.jit
def triton_poi_fused_copy_new_zeros_select_scatter_slice_slice_scatter_5(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp17 = tmp16 < tmp7
    tmp18 = tmp17 & tmp15
    tmp19 = tl.load(in_ptr0 + ((262144*(((787712 + x0 + (513*x1)) // 262656) % 3)) + (786432*(((787712 + x0 + (513*x1) + (787968*x3)) // 787968) % 48)) + ((787712 + x0 + (513*x1)) % 262656)), tmp18, eviction_policy='evict_last', other=0.0)
    ...
    tmp30 = tmp29 < tmp7
    tmp31 = tmp30 & tmp28
    tmp32 = tl.load(in_ptr0 + ((262144*((((-256) + x0 + (513*x1) + (262656*x2) + (787968*x3)) // 262656) % 144)) + (((-256) + x0 + (513*x1) + (262656*x2) + (787968*x3)) % 262656)), tmp31, other=0.0)
