...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/bfloat16/ue/cuer7mzd6expzhagif2wbkrtqo6ep4rs7g7d75tq46ca7so5uagd.py
# Source Nodes: [group_norm_27], Original ATen: [aten.native_group_norm]
# group_norm_27 => convert_element_type_134, var_mean_27
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*bf16', 1: '*bf16', 2: '*bf16', 3: '*bf16', 4: '*fp32', 5: '*fp32', 6: '*fp32', 7: 'i32', 8: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused_native_group_norm_42', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5, 6, 7), equal_to_1=())]}
)
@triton.jit
def triton_red_fused_native_group_norm_42(in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr0, out_ptr1, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp4 = tl.load(in_ptr1 + ((75264*x1) + ((r2 + (15053*x0)) % 75264)), rmask & tmp2 & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp5 = tl.load(in_ptr2 + ((75264*x1) + ((r2 + (15053*x0)) % 75264)), rmask & tmp2 & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp6 = tmp4 - tmp5
        tmp7 = tl.load(in_ptr3 + (((r2 + (15053*x0)) // 196) % 384), rmask & tmp2 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        tmp8 = tmp6 * tmp7
        tmp9 = tmp3 + tmp8
