...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/float32/5i/c5iuqt7mqe7bojnu52fbnvj7fyuzc342ftz5i6swhwsva2fnejfq.py
...
@pointwise(size_hints=[4194304], filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_avg_pool2d_8', 'configs': [AttrsDescriptor(divisible_by_16=(0, 2), equal_to_1=())]})
@triton.jit
def triton_poi_fused_avg_pool2d_8(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp9 = tmp7 & tmp8
    tmp10 = tmp5 & tmp9
    tmp11 = tl.load(in_ptr0 + ((-11700) + x0 + (416*x1) + (23296*x5)), tmp10, other=0.0)
    ...
    tmp16 = tmp14 & tmp15
    tmp17 = tmp5 & tmp16
    tmp18 = tl.load(in_ptr0 + ((-11492) + x0 + (416*x1) + (23296*x5)), tmp17, other=0.0)
    ...
    tmp24 = tmp22 & tmp23
    tmp25 = tmp5 & tmp24
    tmp26 = tl.load(in_ptr0 + ((-11284) + x0 + (416*x1) + (23296*x5)), tmp25, other=0.0)
    ...
    tmp32 = tmp30 & tmp31
    tmp33 = tmp32 & tmp9
    tmp34 = tl.load(in_ptr0 + ((-52) + x0 + (416*x1) + (23296*x5)), tmp33, other=0.0)
    tmp35 = tl.where(tmp33, tmp34, 0.0)
    tmp36 = tmp35 + tmp28
    tmp37 = tmp32 & tmp16
    tmp38 = tl.load(in_ptr0 + (156 + x0 + (416*x1) + (23296*x5)), tmp37, other=0.0)
    tmp39 = tl.where(tmp37, tmp38, 0.0)
    tmp40 = tmp39 + tmp36
    tmp41 = tmp32 & tmp24
    tmp42 = tl.load(in_ptr0 + (364 + x0 + (416*x1) + (23296*x5)), tmp41, other=0.0)
    ...
    tmp48 = tmp46 & tmp47
    tmp49 = tmp48 & tmp9
    tmp50 = tl.load(in_ptr0 + (11596 + x0 + (416*x1) + (23296*x5)), tmp49, other=0.0)
    tmp51 = tl.where(tmp49, tmp50, 0.0)
    tmp52 = tmp51 + tmp44
    tmp53 = tmp48 & tmp16
    tmp54 = tl.load(in_ptr0 + (11804 + x0 + (416*x1) + (23296*x5)), tmp53, other=0.0)
    tmp55 = tl.where(tmp53, tmp54, 0.0)
    tmp56 = tmp55 + tmp52
    tmp57 = tmp48 & tmp24
    tmp58 = tl.load(in_ptr0 + (12012 + x0 + (416*x1) + (23296*x5)), tmp57, other=0.0)
