...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float16/77/c77ltxdegdp76sc7dps64kjwsresjnkc2ezr2yopmjl67rcytkaz.py
# Source Nodes: [float_27, softmax_24, type_as_24], Original ATen: [aten._softmax, aten._to_copy]
# float_27 => convert_element_type_253
# softmax_24 => amax_24, div_28, exp_24, sub_29, sum_25
# type_as_24 => convert_element_type_254
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*fp16', 1: '*fp16', 2: '*fp32', 3: '*fp16', 4: 'i32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax__to_copy_2', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax__to_copy_2(in_ptr0, in_ptr1, out_ptr1, out_ptr3, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp19 = tl.load(in_ptr1 + (x1 + (16*tmp18)), None, eviction_policy='evict_last').to(tl.float32)
        tmp20 = r2
        tmp21 = x0
        tmp22 = tmp20 <= tmp21
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r2 = rindex
        tmp35 = tl.load(in_ptr0 + (r2 + (512*x3)), rmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        ...
        tmp54 = tl.load(in_ptr1 + (x1 + (16*tmp53)), None, eviction_policy='evict_first').to(tl.float32)
        tmp55 = r2
        tmp56 = x0
        tmp57 = tmp55 <= tmp56
        ...
        tl.store(out_ptr1 + (r2 + (512*x3)), tmp67, rmask)
    tmp70 = tl.sum(_tmp70, 1)[:, None]
    for roffset in range(0, rnumel, RBLOCK):
        rindex = roffset + rbase
        rmask = rindex < rnumel
        r2 = rindex
        tmp72 = tl.load(out_ptr1 + (r2 + (512*x3)), rmask, eviction_policy='evict_first', other=0.0)
        tmp73 = tl.exp(tmp72)
        tmp74 = tmp73 / tmp70
        tmp75 = tmp74.to(tl.float32)
        tl.store(out_ptr3 + (r2 + (512*x3)), tmp75, rmask)
''')
