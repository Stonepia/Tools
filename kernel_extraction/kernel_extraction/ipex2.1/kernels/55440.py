...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/bfloat16/36/c36f6ldjnh7cgym2xuchtwsykuve2h5kuw2cekkgwjrkjwawzxyv.py
...
# nan_to_num_10 => convert_element_type_86, eq_42, eq_43, isnan_10, where_31, where_32, where_33
# softmax_10 => amax_10, convert_element_type_88, div_10, exp_10, sub_32, sum_11
# type_as_12 => convert_element_type_89
triton_red_fused__softmax__to_copy_add_nan_to_num_42 = async_compile.triton('triton_red_fused__softmax__to_copy_add_nan_to_num_42', '''
import triton
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*bf16', 1: '*bf16', 2: '*bf16', 3: 'i32', 4: 'i32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax__to_copy_add_nan_to_num_42', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 4), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax__to_copy_add_nan_to_num_42(in_ptr0, in_ptr1, out_ptr2, ks0, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 16
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp0 = tl.load(in_ptr0 + (r1 + (11*x0) + (ks0*x0)), rmask & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp20 = tl.load(in_ptr0 + (r1 + (11*x0) + (ks0*x0)), rmask & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp42 = tl.load(in_ptr0 + (r1 + (11*x0) + (ks0*x0)), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        ...
        tl.store(out_ptr2 + (r1 + (11*x0) + (ks0*x0)), tmp62, rmask & xmask)
''')
