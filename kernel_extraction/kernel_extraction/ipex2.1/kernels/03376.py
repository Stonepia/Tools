...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/amp_bf16/6v/c6vmvryoazdqhlardci7jramuucn6glsq5sbgx7wrs5jf6v6tpyc.py
...
@pointwise(size_hints=[262144], filename=__file__, meta={'signature': {0: '*i32', 1: '*bf16', 2: '*bf16', 3: '*bf16', 4: '*bf16', 5: '*bf16', 6: '*bf16', 7: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_cat_9', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5, 6, 7), equal_to_1=())]})
@triton.jit
def triton_poi_fused_cat_9(in_ptr0, in_ptr1, in_ptr2, out_ptr0, out_ptr1, out_ptr2, out_ptr3, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp5 = tl.load(in_ptr1 + (x0 + (64*((tmp4 // 64) % 12)) + (768*(x1 % 64)) + (49152*(tmp4 % 64))), None).to(tl.float32)
    ...
    tmp11 = tl.load(in_ptr1 + (x0 + (64*((tmp10 // 64) % 12)) + (768*(x1 % 64)) + (49152*(tmp10 % 64))), None).to(tl.float32)
    tmp12 = tl.load(in_ptr2 + (x0 + (64*((tmp4 // 64) % 12)) + (768*(x1 % 64)) + (49152*(tmp4 % 64))), None).to(tl.float32)
    tmp13 = tl.load(in_ptr2 + (x0 + (64*((tmp10 // 64) % 12)) + (768*(x1 % 64)) + (49152*(tmp10 % 64))), None).to(tl.float32)
