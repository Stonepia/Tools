...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/bfloat16/s6/cs6yxayzy7xh7f6dquu2onwqm5isag3nywpu5zzpylidaybp5aiw.py
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*bf16', 1: '*bf16', 2: '*i64', 3: '*fp32', 4: '*i1', 5: '*fp32', 6: '*bf16', 7: 'i32', 8: 'i32', 9: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax__to_copy_native_dropout_4', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5, 6, 8, 9), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax__to_copy_native_dropout_4(in_ptr0, in_ptr1, in_ptr2, out_ptr2, out_ptr3, out_ptr4, out_ptr5, load_seed_offset, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r3 = rindex
        tmp33 = tl.load(in_ptr0 + (r3 + (1024*x4)), rmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        tmp34 = r3 + ((-1)*x0)
        ...
        tmp58 = tl.load(in_ptr1 + (x1 + (8*tmp57)), rmask, eviction_policy='evict_last').to(tl.float32)
        tmp59 = 0.0
        tmp60 = tmp58 + tmp59
        tmp61 = tmp33 + tmp60
        ...
        tl.store(out_ptr2 + (r3 + (1024*x4)), tmp70, rmask)
    tmp66 = tl.sum(_tmp66, 1)[:, None]
    for roffset in range(0, rnumel, RBLOCK):
        rindex = roffset + rbase
        rmask = rindex < rnumel
        r3 = rindex
        tmp71 = tl.load(out_ptr2 + (r3 + (1024*x4)), rmask, eviction_policy='evict_first', other=0.0)
        tmp75 = tl.load(in_ptr0 + (r3 + (1024*x4)), rmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        ...
        tmp100 = tl.load(in_ptr1 + (x1 + (8*tmp99)), rmask, eviction_policy='evict_first').to(tl.float32)
        tmp101 = 0.0
        ...
        tl.store(out_ptr4 + (r3 + (1024*x4)), tmp107, rmask)
        tl.store(out_ptr5 + (r3 + (1024*x4)), tmp112, rmask)
''')
