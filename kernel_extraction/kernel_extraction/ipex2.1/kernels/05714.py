...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/bfloat16/sj/csjq7gj2iy5eltytjow67cwls7etcq3whcrg37mcwcgxzdtmvoyr.py
# Source Nodes: [add_9, mul_1, softmax_1], Original ATen: [aten._softmax, aten.add, aten.mul]
# add_9 => add_59
# mul_1 => mul_76
# softmax_1 => amax_1, convert_element_type_77, convert_element_type_78, div_1, exp_1, sub_26, sum_2
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*bf16', 1: '*bf16', 2: '*bf16', 3: '*fp32', 4: '*bf16', 5: 'i32', 6: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax_add_mul_13', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5, 6), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax_add_mul_13(in_ptr0, in_ptr1, in_ptr2, out_ptr1, out_ptr3, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 131072
    rnumel = 256
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp35 = tmp33 < tmp34
        tmp36 = tmp35 & tmp32
        tmp37 = tl.load(in_ptr1 + ((31*((15 + (31*(x0 // 16)) + (r2 // 16)) // 32)) + (496*(x0 % 16)) + (7936*((((16*x1) + (x0 % 16)) // 16) % 512)) + ((15 + (31*(x0 // 16)) + (r2 // 16)) % 32)), rmask & tmp36, eviction_policy='evict_first', other=0.0).to(tl.float32)
