...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/amp_fp16/hl/chlikush4sretygqe3jbyxrns45rvi6jymbqix7n34yclo7tp3zm.py
# Source Nodes: [getattr_getattr_l__self___stages___2___blocks___0___token_mixer_pool], Original ATen: [aten.avg_pool2d]
# getattr_getattr_l__self___stages___2___blocks___0___token_mixer_pool => avg_pool2d_12
...
@pointwise(size_hints=[8388608], filename=__file__, meta={'signature': {0: '*fp16', 1: '*fp16', 2: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_avg_pool2d_34', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2), equal_to_1=())]})
@triton.jit
def triton_poi_fused_avg_pool2d_34(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp9 = tmp7 & tmp8
    tmp10 = tmp5 & tmp9
    tmp11 = tl.load(in_ptr0 + ((-5760) + x6), tmp10, other=0.0).to(tl.float32)
    ...
    tmp24 = tmp22 & tmp23
    tmp25 = tmp5 & tmp24
    tmp26 = tl.load(in_ptr0 + ((-4992) + x6), tmp25, other=0.0).to(tl.float32)
    ...
    tmp32 = tmp30 & tmp31
    tmp33 = tmp32 & tmp9
    tmp34 = tl.load(in_ptr0 + ((-384) + x6), tmp33, other=0.0).to(tl.float32)
    ...
    tmp40 = tmp39 + tmp36
    tmp41 = tmp32 & tmp24
    tmp42 = tl.load(in_ptr0 + (384 + x6), tmp41, other=0.0).to(tl.float32)
    ...
    tmp48 = tmp46 & tmp47
    tmp49 = tmp48 & tmp9
    tmp50 = tl.load(in_ptr0 + (4992 + x6), tmp49, other=0.0).to(tl.float32)
    ...
    tmp56 = tmp55 + tmp52
    tmp57 = tmp48 & tmp24
    tmp58 = tl.load(in_ptr0 + (5760 + x6), tmp57, other=0.0).to(tl.float32)
