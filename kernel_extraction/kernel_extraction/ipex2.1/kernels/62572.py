...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/float32/uc/cucvgavkni7fvzrhtyrqz4scorglp7vknzpvrrsilv2ancfrea6v.py
...
@pointwise(size_hints=[32768, 2048], tile_hint=TileHint.SQUARE,filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: 'i32', 3: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_avg_pool2d_3', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2), equal_to_1=())]})
@triton.jit
def triton_poi_fused_avg_pool2d_3(in_ptr0, out_ptr0, ynumel, xnumel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    ...
    tmp16 = tmp14 & tmp15
    tmp17 = tmp5 & tmp16
    tmp18 = tl.load(in_ptr0 + ((-6720) + y0 + (192*x5) + (235200*y1)), tmp17 & xmask, eviction_policy='evict_last', other=0.0)
    ...
    tmp24 = tmp22 & tmp23
    tmp25 = tmp5 & tmp24
    tmp26 = tl.load(in_ptr0 + ((-6528) + y0 + (192*x5) + (235200*y1)), tmp25 & xmask, eviction_policy='evict_last', other=0.0)
    ...
    tmp32 = tmp30 & tmp31
    tmp33 = tmp32 & tmp9
    tmp34 = tl.load(in_ptr0 + ((-192) + y0 + (192*x5) + (235200*y1)), tmp33 & xmask, eviction_policy='evict_last', other=0.0)
    tmp35 = tl.where(tmp33, tmp34, 0.0)
    tmp36 = tmp35 + tmp28
    tmp37 = tmp32 & tmp16
    tmp38 = tl.load(in_ptr0 + (y0 + (192*x5) + (235200*y1)), tmp37 & xmask, eviction_policy='evict_last', other=0.0)
    tmp39 = tl.where(tmp37, tmp38, 0.0)
    tmp40 = tmp39 + tmp36
    tmp41 = tmp32 & tmp24
    tmp42 = tl.load(in_ptr0 + (192 + y0 + (192*x5) + (235200*y1)), tmp41 & xmask, eviction_policy='evict_last', other=0.0)
    ...
    tmp48 = tmp46 & tmp47
    tmp49 = tmp48 & tmp9
    tmp50 = tl.load(in_ptr0 + (6528 + y0 + (192*x5) + (235200*y1)), tmp49 & xmask, eviction_policy='evict_last', other=0.0)
    tmp51 = tl.where(tmp49, tmp50, 0.0)
    tmp52 = tmp51 + tmp44
    tmp53 = tmp48 & tmp16
    tmp54 = tl.load(in_ptr0 + (6720 + y0 + (192*x5) + (235200*y1)), tmp53 & xmask, eviction_policy='evict_last', other=0.0)
    tmp55 = tl.where(tmp53, tmp54, 0.0)
    tmp56 = tmp55 + tmp52
    tmp57 = tmp48 & tmp24
    tmp58 = tl.load(in_ptr0 + (6912 + y0 + (192*x5) + (235200*y1)), tmp57 & xmask, eviction_policy='evict_last', other=0.0)
