...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float32/b3/cb3txeiqlfhvzusvzhwvbii3bhyo7zn5v64wocxk26jrzcwujrmd.py
...
@pointwise(size_hints=[1024, 4096], tile_hint=TileHint.DEFAULT,filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: '*fp32', 3: 'i32', 4: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_copy_new_zeros_select_scatter_slice_slice_scatter_11', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 4), equal_to_1=())]})
@triton.jit
def triton_poi_fused_copy_new_zeros_select_scatter_slice_slice_scatter_11(in_ptr0, in_ptr1, out_ptr0, ynumel, xnumel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    ...
    tmp8 = tmp6 < tmp7
    tmp9 = tmp8 & tmp5
    tmp10 = tl.load(in_ptr0 + ((256*((3808256 + y0 + (513*x1)) // 262656)) + (((3808256 + y0 + (513*x1)) // 512) % 513)), tmp9 & ymask, eviction_policy='evict_last', other=0.0)
    tmp11 = tl.load(in_ptr1 + ((256*((3808256 + y0 + (513*x1)) // 262656)) + ((y0 + (513*x1)) % 512)), tmp9 & ymask, eviction_policy='evict_last', other=0.0)
    tmp12 = tmp10 * tmp11
    ...
    tmp21 = tmp20 < tmp7
    tmp22 = tmp21 & tmp19
    tmp23 = tl.load(in_ptr0 + ((256*(((3939584 + y0 + (513*x1)) // 262656) % 15)) + (((3939584 + y0 + (513*x1)) // 512) % 513)), tmp22 & ymask, eviction_policy='evict_last', other=0.0)
    tmp24 = tl.load(in_ptr1 + ((256*(((3939584 + y0 + (513*x1)) // 262656) % 15)) + ((3939584 + y0 + (513*x1)) % 512)), tmp22 & ymask, eviction_policy='evict_last', other=0.0)
    tmp25 = tmp23 * tmp24
    ...
    tmp36 = tmp35 < tmp7
    tmp37 = tmp36 & tmp34
    tmp38 = tl.load(in_ptr0 + ((256*((((-256) + y0 + (513*x1) + (262656*x2)) // 262656) % 15)) + ((((-256) + y0 + (513*x1) + (262656*x2)) // 512) % 513)), tmp37 & ymask, eviction_policy='evict_last', other=0.0)
    tmp39 = tl.load(in_ptr1 + ((256*((((-256) + y0 + (513*x1) + (262656*x2)) // 262656) % 15)) + (((-256) + y0 + (513*x1) + (262656*x2)) % 512)), tmp37 & ymask, eviction_policy='evict_last', other=0.0)
    tmp40 = tmp38 * tmp39
