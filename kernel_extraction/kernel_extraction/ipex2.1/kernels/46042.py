...
# Original file: ./detectron2_maskrcnn_r_101_fpn__79_inference_119.59/detectron2_maskrcnn_r_101_fpn__79_inference_119.59_0.py
...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/bfloat16/tf/ctfbsmtfmvplyze5f4w743xihkoaopfulblepaakzivqavbjkl4a.py
...
@pointwise(size_hints=[16777216], filename=__file__, meta={'signature': {0: '*fp32', 1: '*bf16', 2: '*i1', 3: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_ge_grid_sampler_2d_index_2', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3), equal_to_1=())]})
@triton.jit
def triton_poi_fused_ge_grid_sampler_2d_index_2(in_ptr0, in_ptr1, out_ptr8, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp26 = tl.load(in_ptr1 + (tmp25 + (28*tmp22) + (784*x1)), xmask).to(tl.float32)
    ...
    tmp40 = tl.where(tmp39 < 0, tmp39 + 28, tmp39)
    # tl.device_assert((0 <= tmp40) & (tmp40 < 28), "index out of bounds: 0 <= tmp40 < 28")
    tmp41 = tmp29.to(tl.int64)
    tmp42 = tl.where(tmp38, tmp41, tmp20)
    tmp43 = tl.where(tmp42 < 0, tmp42 + 28, tmp42)
    # tl.device_assert((0 <= tmp43) & (tmp43 < 28), "index out of bounds: 0 <= tmp43 < 28")
    tmp44 = tl.load(in_ptr1 + (tmp43 + (28*tmp40) + (784*x1)), xmask).to(tl.float32)
    ...
    tmp58 = tl.where(tmp57 < 0, tmp57 + 28, tmp57)
    # tl.device_assert((0 <= tmp58) & (tmp58 < 28), "index out of bounds: 0 <= tmp58 < 28")
    tmp59 = tl.load(in_ptr1 + (tmp58 + (28*tmp56) + (784*x1)), xmask).to(tl.float32)
    ...
    tmp73 = tmp60 * tmp63
    tmp74 = tmp72 + tmp73
    tmp75 = tl.where(tmp66 < 0, tmp66 + 28, tmp66)
    # tl.device_assert((0 <= tmp75) & (tmp75 < 28), "index out of bounds: 0 <= tmp75 < 28")
    tmp76 = tl.where(tmp67 < 0, tmp67 + 28, tmp67)
    # tl.device_assert((0 <= tmp76) & (tmp76 < 28), "index out of bounds: 0 <= tmp76 < 28")
    tmp77 = tl.load(in_ptr1 + (tmp76 + (28*tmp75) + (784*x1)), xmask).to(tl.float32)
