...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/amp_bf16/wb/cwb6iclato4gji7i2hvyjsgjuc2udy6ymbz7rcie2nfbnvqofwbg.py
...
def triton_poi_fused_add_mul_sub_38(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, out_ptr0, ynumel, xnumel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    ...
    tmp2 = tl.load(in_ptr1 + (x2 + (384*y3)), xmask & ymask, eviction_policy='evict_last').to(tl.float32)
    ...
    tmp9 = tl.load(in_ptr4 + (x2 + (384*y3)), xmask & ymask, eviction_policy='evict_last').to(tl.float32)
    ...
    tmp12 = tmp10 * tmp11
    tmp13 = tmp8 + tmp12
    tl.store(out_ptr0 + (y0 + (196*x2) + (75264*y1)), tmp13, xmask & ymask)
''')
