...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float32/u3/cu3nr5qzg7g3yrzwylfdzygu46p6yzfawkbtvqzj4hc4gagbbd2m.py
...
def triton_red_fused__softmax_5(in_ptr0, in_ptr1, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp25 = tl.load(in_ptr1 + (x1 + (12*tmp24)), rmask, eviction_policy='evict_last')
        tmp26 = tmp0 + tmp25
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r2 = rindex
        tmp30 = tl.load(in_ptr0 + (r2 + (2048*x3)), rmask, eviction_policy='evict_last', other=0.0)
        tmp31 = r2 + ((-1)*x0)
        ...
        tmp55 = tl.load(in_ptr1 + (x1 + (12*tmp54)), rmask, eviction_policy='evict_last')
        tmp56 = tmp30 + tmp55
        tmp57 = tmp56 - tmp28
        tmp58 = tl.exp(tmp57)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r2 = rindex
        tmp62 = tl.load(in_ptr0 + (r2 + (2048*x3)), rmask, eviction_policy='evict_first', other=0.0)
        tmp63 = r2 + ((-1)*x0)
        ...
        tmp87 = tl.load(in_ptr1 + (x1 + (12*tmp86)), rmask, eviction_policy='evict_first')
        tmp88 = tmp62 + tmp87
        tmp89 = tmp88 - tmp28
        tmp90 = tl.exp(tmp89)
        tmp91 = tmp90 / tmp60
        tl.store(out_ptr2 + (r2 + (2048*x3)), tmp91, rmask)
''')
