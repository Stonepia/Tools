...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/float32/4o/c4orn4f65zc7x56vucqqusgqsilmc5ao6oqgnilajc5qvrmqtdks.py
...
@pointwise(size_hints=[4194304], filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: '*fp32', 3: '*fp32', 4: '*fp32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused__to_copy_full_like_where_9', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5), equal_to_1=())]})
@triton.jit
def triton_poi_fused__to_copy_full_like_where_9(in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    x3 = (xindex // 789504)
    x7 = xindex
    tmp0 = tl.load(in_ptr0 + (x5), None, eviction_policy='evict_last')
    tmp4 = tl.load(in_ptr1 + (x0 + (513*x6)), None)
    tmp22 = tl.load(in_ptr3 + (x0 + (513*x1) + (525312*x4)), None)
    tmp1 = (tmp0 != 0)
    ...
    tmp14 = tmp12 < tmp13
    tmp15 = tmp14 & tmp11
    tmp16 = tl.load(in_ptr2 + ((512*((((-131584) + x0 + (513*x1) + (787968*x2) + (9455616*x3)) // 512) % 513)) + (262144*((((-131584) + x0 + (513*x1) + (787968*x2) + (9455616*x3)) // 262656) % 144)) + ((x0 + (513*x1)) % 512)), tmp15, other=0.0)
    tmp17 = tl.where(tmp15, tmp16, 0.0)
    tmp18 = tl.where(tmp11, tmp17, 0.0)
    tmp19 = tl.load(in_ptr3 + (x0 + (513*x1) + (525312*x4)), tmp7, other=0.0)
