...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/bfloat16/2q/c2qsnfvpqszqlsdjoxwy26gdajjjcf4upqqbpxyh3xhe6kye2tp5.py
# Source Nodes: [add_5, add_6, getattr_getattr_l__mod___levels___1___transformer_encoder___0___attn_proj_drop, layer_norm_6], Original ATen: [aten.add, aten.clone, aten.native_layer_norm]
# add_5 => add_17
# add_6 => add_20
# getattr_getattr_l__mod___levels___1___transformer_encoder___0___attn_proj_drop => clone_21
# layer_norm_6 => add_21, add_22, convert_element_type_22, convert_element_type_23, mul_24, mul_25, rsqrt_6, sub_9, var_mean_6
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*bf16', 1: '*bf16', 2: '*bf16', 3: '*bf16', 4: '*bf16', 5: '*bf16', 6: 'i32', 7: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused_add_clone_native_layer_norm_21', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5, 6, 7), equal_to_1=())]}
)
@triton.jit
def triton_red_fused_add_clone_native_layer_norm_21(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 25088
    rnumel = 256
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r3 = rindex
        tmp10 = tl.load(in_ptr0 + ((14*(x1 % 2)) + (28*(x0 // 14)) + (392*(x1 // 2)) + (784*r3) + (200704*x2) + (x0 % 14)), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp11 = tl.load(in_ptr1 + (r3 + (256*x4)), rmask & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        tmp13 = tl.load(in_ptr2 + (r3 + (256*x5)), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
