...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float32/um/cumg7jbowrknhnyonvvdxxlkjpras7pxcs2t6ga3hasdw7avyiox.py
...
# l__self___fpn_out_branches_3_3 => _unsafe_index_24, _unsafe_index_25, _unsafe_index_26, _unsafe_index_27, add_173, add_174, add_175, convert_element_type_160, mul_245, mul_247, mul_248, mul_249, mul_250, mul_251, mul_252, sub_85, sub_86, sub_87, sub_88
...
@pointwise(size_hints=[4194304], filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused__to_copy__unsafe_index_add_arange_mul_rsub_sub_8', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2), equal_to_1=())]})
@triton.jit
def triton_poi_fused__to_copy__unsafe_index_add_arange_mul_rsub_sub_8(in_ptr0, out_ptr2, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp15 = tl.load(in_ptr0 + (x2 + (64*tmp14) + (2048*tmp8)), None)
    ...
    tmp24 = tl.load(in_ptr0 + (x2 + (64*tmp14) + (2048*tmp23)), None)
    tmp25 = tmp24 * tmp17
    tmp26 = tmp19 + tmp25
    tmp27 = libdevice.ceil(tmp13)
    tmp28 = triton_helpers.minimum(tmp27, tmp21)
    tmp29 = tmp28.to(tl.int32)
    tmp30 = tl.load(in_ptr0 + (x2 + (64*tmp29) + (2048*tmp8)), None)
    tmp31 = tmp30 * tmp18
    tmp32 = tl.load(in_ptr0 + (x2 + (64*tmp29) + (2048*tmp23)), None)
    tmp33 = tmp32 * tmp17
    tmp34 = tmp31 + tmp33
