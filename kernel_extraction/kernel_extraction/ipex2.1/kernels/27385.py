...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/bfloat16/nv/cnvd3amekxw5nu22ujnpcdfeeawpavexsnm2hh6qn4zlseezjd4d.py
# Source Nodes: [iadd_1, nan_to_num, softmax, type_as_2], Original ATen: [aten._softmax, aten._to_copy, aten.add, aten.nan_to_num]
# iadd_1 => add_4
# nan_to_num => convert_element_type_6, eq_2, eq_3, full_default_2, full_default_3, full_default_4, isnan, where_1, where_2, where_3
# softmax => amax, convert_element_type_8, div, exp, sub_2, sum_1
# type_as_2 => convert_element_type_9
triton_red_fused__softmax__to_copy_add_nan_to_num_6 = async_compile.triton('triton_red_fused__softmax__to_copy_add_nan_to_num_6', '''
import triton
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*bf16', 1: '*bf16', 2: '*bf16', 3: 'i32', 4: 'i32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax__to_copy_add_nan_to_num_6', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 4), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax__to_copy_add_nan_to_num_6(in_ptr0, in_ptr1, out_ptr2, ks0, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 16
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp20 = tl.load(in_ptr0 + (r1 + x0 + (ks0*x0)), rmask & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp42 = tl.load(in_ptr0 + (r1 + x0 + (ks0*x0)), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        ...
        tl.store(out_ptr2 + (r1 + x0 + (ks0*x0)), tmp62, rmask & xmask)
''')
