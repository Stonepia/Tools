...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/amp_fp16/ak/cakxg36ga6azxbctbd23bgpeqsizomtncbtmwfxjtwaw24cn5qrh.py
...
# truediv_73 => div_70
# truediv_74 => div_71
triton_poi_fused_clone_copy_div_mul_neg_select_scatter_slice_slice_scatter_sub_85 = async_compile.triton('triton_poi_fused_clone_copy_div_mul_neg_select_scatter_slice_slice_scatter_sub_85', '''
import triton
...
@pointwise(size_hints=[4194304], filename=__file__, meta={'signature': {0: '*fp64', 1: '*fp64', 2: '*fp64', 3: '*fp64', 4: '*fp64', 5: '*fp64', 6: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_clone_copy_div_mul_neg_select_scatter_slice_slice_scatter_sub_85', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5, 6), equal_to_1=())]})
@triton.jit
def triton_poi_fused_clone_copy_div_mul_neg_select_scatter_slice_slice_scatter_sub_85(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    xnumel = 3246048
    ...
    x5 = (xindex // 3) % 5304
    x6 = xindex
    tmp38 = tl.load(in_ptr1 + (x0 + (3*x4)), xmask, eviction_policy='evict_last')
    tmp39 = tl.load(in_ptr2 + (x0 + (3*x4)), xmask, eviction_policy='evict_last')
    tmp48 = tl.load(in_ptr4 + (x6), xmask)
    tmp0 = x1
    ...
    tmp8 = tmp6 == tmp7
    tmp9 = tl.load(in_ptr0 + ((-1) + x1 + (24*x4)), tmp5 & xmask, eviction_policy='evict_last', other=0.0)
    tmp10 = tmp0 == tmp7
    tmp11 = tl.load(in_ptr1 + (x0 + (3*x4)), tmp5 & xmask, eviction_policy='evict_last', other=0.0)
    tmp12 = tl.load(in_ptr2 + (x0 + (3*x4)), tmp5 & xmask, eviction_policy='evict_last', other=0.0)
    tmp13 = x3
    ...
    tmp23 = tmp21 & tmp22
    tmp24 = tmp23 & tmp19
    tmp25 = tl.load(in_ptr3 + ((-10452) + x5 + (5200*x3)), tmp24 & xmask, eviction_policy='evict_last', other=0.0)
    tmp26 = tl.load(in_ptr4 + (x6), tmp24 & xmask, other=0.0)
    tmp27 = tl.where(tmp8, tmp25, tmp26)
    tmp28 = tl.where(tmp24, tmp27, 0.0)
    tmp29 = tl.load(in_ptr4 + (x6), tmp19 & xmask, other=0.0)
    tmp30 = tl.where(tmp23, tmp28, tmp29)
    tmp31 = tl.where(tmp19, tmp30, 0.0)
    tmp32 = tl.load(in_ptr4 + (x6), tmp5 & xmask, other=0.0)
    ...
    tmp40 = tmp23 & tmp18
    tmp41 = tl.load(in_ptr3 + ((-10452) + x5 + (5200*x3)), tmp40 & xmask, eviction_policy='evict_last', other=0.0)
    tmp42 = tl.load(in_ptr4 + (x6), tmp40 & xmask, other=0.0)
    tmp43 = tl.where(tmp8, tmp41, tmp42)
    tmp44 = tl.where(tmp40, tmp43, 0.0)
    tmp45 = tl.load(in_ptr4 + (x6), tmp18 & xmask, other=0.0)
