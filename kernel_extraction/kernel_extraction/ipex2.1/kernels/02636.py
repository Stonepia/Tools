...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/float32/cf/ccfv4hka2r4ogqk7pw7h3mltalwknm5jrfx5hy73uwrqnskt7wrj.py
# Source Nodes: [add_13, mul_8, softmax_2], Original ATen: [aten._softmax, aten.add, aten.mul]
# add_13 => add_75
# mul_8 => mul_129
# softmax_2 => amax_2, div_2, exp_2, sub_33, sum_3
triton_red_fused__softmax_add_mul_35 = async_compile.triton('triton_red_fused__softmax_add_mul_35', '''
import triton
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*fp32', 1: '*fp32', 2: '*fp32', 3: '*fp32', 4: 'i32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax_add_mul_35', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax_add_mul_35(in_ptr0, in_ptr1, in_ptr2, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 65536
    rnumel = 256
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp8 = tmp6 < tmp7
        tmp9 = tmp8 & tmp5
        tmp10 = tl.load(in_ptr1 + ((31*((15 + (31*(x0 // 16)) + (r2 // 16)) // 32)) + (496*(x0 % 16)) + (7936*((((16*x1) + (x0 % 16)) // 16) % 256)) + ((15 + (31*(x0 // 16)) + (r2 // 16)) % 32)), rmask & tmp9, eviction_policy='evict_last', other=0.0)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp34 = tmp32 < tmp33
        tmp35 = tmp34 & tmp31
        tmp36 = tl.load(in_ptr1 + ((31*((15 + (31*(x0 // 16)) + (r2 // 16)) // 32)) + (496*(x0 % 16)) + (7936*((((16*x1) + (x0 % 16)) // 16) % 256)) + ((15 + (31*(x0 // 16)) + (r2 // 16)) % 32)), rmask & tmp35, eviction_policy='evict_last', other=0.0)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp62 = tmp60 < tmp61
        tmp63 = tmp62 & tmp59
        tmp64 = tl.load(in_ptr1 + ((31*((15 + (31*(x0 // 16)) + (r2 // 16)) // 32)) + (496*(x0 % 16)) + (7936*((((16*x1) + (x0 % 16)) // 16) % 256)) + ((15 + (31*(x0 // 16)) + (r2 // 16)) % 32)), rmask & tmp63, eviction_policy='evict_first', other=0.0)
