...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/float32/ja/cjapoc3iypb7g43sy7wi4mnj64hjyvqyroojvgl5w6o3jieukv57.py
# Source Nodes: [add, add_1, add_2, iadd, int_1, l__mod___roberta_embeddings_dropout, l__mod___roberta_embeddings_layer_norm, l__mod___roberta_embeddings_position_embeddings, l__mod___roberta_embeddings_token_type_embeddings, l__mod___roberta_embeddings_word_embeddings, l__mod___roberta_encoder_layer_0_attention_self_query, long, mul_1, ne, type_as], Original ATen: [aten._to_copy, aten.add, aten.embedding, aten.mul, aten.native_dropout, aten.native_layer_norm, aten.native_layer_norm_backward, aten.ne, aten.view]
# add => add
# add_1 => add_1
# add_2 => add_2
# iadd => add_3
...
# l__mod___roberta_embeddings_layer_norm => add_4, add_5, mul_2, mul_3, rsqrt, sub_1, var_mean
...
# l__mod___roberta_encoder_layer_0_attention_self_query => view
...
def triton_per_fused__to_copy_add_embedding_mul_native_dropout_native_layer_norm_native_layer_norm_backward_ne_view_1(in_out_ptr0, in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, in_ptr6, in_ptr7, out_ptr4, out_ptr5, out_ptr6, out_ptr7, load_seed_offset, xnumel, rnumel):
    ...
    tmp15 = tl.load(in_ptr3 + (r3), rmask, eviction_policy='evict_last', other=0.0)
    ...
    tmp10 = tmp9 + tmp5
    tmp11 = tl.where(tmp4 < 0, tmp4 + 32005, tmp4)
    # tl.device_assert((0 <= tmp11) & (tmp11 < 32005), "index out of bounds: 0 <= tmp11 < 32005")
    tmp12 = tl.load(in_ptr1 + (r3 + (768*tmp11)), rmask, other=0.0)
    tmp14 = tl.where(tmp13 < 0, tmp13 + 1, tmp13)
    # tl.device_assert((0 <= tmp14) & (tmp14 < 1), "index out of bounds: 0 <= tmp14 < 1")
    tmp16 = tmp12 + tmp15
    tmp17 = tl.where(tmp10 < 0, tmp10 + 514, tmp10)
    # tl.device_assert((0 <= tmp17) & (tmp17 < 514), "index out of bounds: 0 <= tmp17 < 514")
    ...
    tl.store(out_ptr6 + (r3 + (768*x0)), tmp55, rmask)
    tl.store(out_ptr7 + (x0), tmp56, None)
''')
