...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float16/2c/c2caa6mvbsavrue5zprd4aqimavghhvawnmh6lzvwlqumhhrursi.py
...
def triton_poi_fused_constant_pad_nd_convolution_div_sub_0(in_ptr0, in_ptr1, in_ptr2, out_ptr1, ynumel, xnumel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    ...
    tmp4 = tmp3 < tmp1
    tmp5 = tmp2 & tmp4
    tmp6 = tl.load(in_ptr0 + (x1 + (256*x2) + (65536*y0)), tmp5 & ymask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    tmp7 = tl.load(in_ptr1 + (tl.broadcast_to(y0, [XBLOCK, YBLOCK])), tmp5 & ymask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    tmp8 = tmp6 - tmp7
    tmp9 = tl.load(in_ptr2 + (tl.broadcast_to(y0, [XBLOCK, YBLOCK])), tmp5 & ymask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    tmp10 = tmp8 / tmp9
