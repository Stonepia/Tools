...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/amp_bf16/ny/cnyhv5baufo5aqzm3e2lr3dvsjhmvemia7bzepv3csh72ztlhjpg.py
...
# nan_to_num_17 => convert_element_type_369, eq_70, eq_71, isnan_17, where_52, where_53, where_54
# softmax_17 => amax_17, div_17, exp_17, sub_53, sum_18
# type_as_19 => convert_element_type_373
triton_red_fused__softmax__to_copy_add_nan_to_num_63 = async_compile.triton('triton_red_fused__softmax__to_copy_add_nan_to_num_63', '''
import triton
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*bf16', 1: '*fp32', 2: '*bf16', 3: 'i32', 4: 'i32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax__to_copy_add_nan_to_num_63', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 4), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax__to_copy_add_nan_to_num_63(in_ptr0, in_ptr1, out_ptr2, ks0, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 16
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp0 = tl.load(in_ptr0 + (r1 + (18*x0) + (ks0*x0)), rmask & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp20 = tl.load(in_ptr0 + (r1 + (18*x0) + (ks0*x0)), rmask & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp42 = tl.load(in_ptr0 + (r1 + (18*x0) + (ks0*x0)), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        ...
        tl.store(out_ptr2 + (r1 + (18*x0) + (ks0*x0)), tmp62, rmask & xmask)
''')
