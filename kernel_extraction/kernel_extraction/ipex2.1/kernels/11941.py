...
def triton_poi_fused_abs_add_div_gt_lift_fresh_lt_maximum_minimum_mul_rsub_scalar_tensor_sub_where_69(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp54 = tl.load(in_ptr3 + (2 + x4), xmask, eviction_policy='evict_last').to(tl.float32)
    tmp55 = tl.load(in_ptr4 + (1 + x1), xmask, eviction_policy='evict_last').to(tl.float32)
    tmp1 = 0.0
    tmp2 = tmp0 > tmp1
    tmp5 = tmp3 - tmp4
    tmp6 = x1
    ...
    tmp49 = triton_helpers.minimum(tmp43, tmp48)
