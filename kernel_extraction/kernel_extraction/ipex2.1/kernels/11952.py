...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float16/ep/cep4pkwq3ubnucdcvet3uze7bowftgrhptwrbvvidotq55krawul.py
...
# truediv_73 => div_70
# truediv_74 => div_71
triton_poi_fused_clone_copy_div_mul_neg_select_scatter_slice_slice_scatter_sub_80 = async_compile.triton('triton_poi_fused_clone_copy_div_mul_neg_select_scatter_slice_slice_scatter_sub_80', '''
import triton
...
@pointwise(size_hints=[4194304], filename=__file__, meta={'signature': {0: '*fp16', 1: '*fp16', 2: '*fp16', 3: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': ['in_out_ptr0'], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_clone_copy_div_mul_neg_select_scatter_slice_slice_scatter_sub_80', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3), equal_to_1=())]})
@triton.jit
def triton_poi_fused_clone_copy_div_mul_neg_select_scatter_slice_slice_scatter_sub_80(in_out_ptr0, in_ptr0, in_ptr1, xnumel, XBLOCK : tl.constexpr):
    xnumel = 3246048
    ...
    x4 = (xindex // 3)
    x5 = xindex
    tmp7 = tl.load(in_ptr0 + (3*x2), xmask, eviction_policy='evict_last').to(tl.float32)
    tmp17 = tl.load(in_ptr1 + (75 + (78*x2)), xmask, eviction_policy='evict_last').to(tl.float32)
    tmp20 = tl.load(in_ptr0 + (x0 + (3*x2)), xmask, eviction_policy='evict_last').to(tl.float32)
    tmp24 = tl.load(in_ptr1 + (75 + x0 + (78*x2)), xmask, eviction_policy='evict_last').to(tl.float32)
    ...
    tmp12 = tmp10 & tmp11
    tmp13 = tmp4 == tmp4
    tmp14 = tl.load(in_ptr1 + (75 + (78*x2)), tmp12 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    tmp21 = tl.load(in_ptr1 + (75 + x0 + (78*x2)), tmp12 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    tmp29 = tmp0 < tmp8
    tmp30 = tmp28 & tmp29
    tmp31 = tl.load(in_ptr1 + (3*x4), tmp30 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    tmp32 = tl.load(in_ptr1 + (x5), tmp30 & xmask, other=0.0).to(tl.float32)
