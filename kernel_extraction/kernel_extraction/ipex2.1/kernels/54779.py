...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/float32/e7/ce7g2yx4omnj4hjsgkmfeayfupvl5xcjaxejl3sdb5ofsvq7tahz.py
...
@pointwise(size_hints=[8388608], filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: '*fp32', 3: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_copy_slice_scatter_6', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3), equal_to_1=())]})
@triton.jit
def triton_poi_fused_copy_slice_scatter_6(in_ptr0, in_ptr1, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    x3 = xindex % 131328
    x4 = xindex
    tmp41 = tl.load(in_ptr1 + (x3 + (525312*x2)), None)
    tmp0 = x1
    ...
    tmp11 = tmp9 < tmp10
    tmp12 = tmp11 & tmp8
    tmp13 = tl.load(in_ptr0 + ((262144*((((-256) + x0 + (513*x1) + (787968*x2)) // 262656) % 144)) + (((-256) + x0 + (513*x1) + (787968*x2)) % 262656)), tmp12, other=0.0)
    ...
    tmp21 = tmp20 < tmp10
    tmp22 = tmp21 & tmp19
    tmp23 = tl.load(in_ptr0 + ((512*((((-131584) + x3 + (787968*x2)) // 512) % 513)) + (262144*((((-131584) + x3 + (787968*x2)) // 262656) % 144)) + (x3 % 512)), tmp22, other=0.0)
    tmp24 = tl.where(tmp22, tmp23, 0.0)
    tmp25 = tl.where(tmp19, tmp24, 0.0)
    tmp26 = tl.load(in_ptr1 + (x3 + (525312*x2)), tmp18, other=0.0)
    tmp27 = tl.where(tmp6, tmp25, tmp26)
    tmp28 = tl.where(tmp18, tmp27, 0.0)
    tmp29 = tl.load(in_ptr1 + (x3 + (525312*x2)), tmp2, other=0.0)
    tmp30 = tl.where(tmp17, tmp28, tmp29)
    tmp31 = tl.where(tmp7, tmp15, tmp30)
    tmp32 = tl.where(tmp2, tmp31, 0.0)
    tmp33 = tmp6 & tmp17
    tmp34 = tmp21 & tmp33
    tmp35 = tl.load(in_ptr0 + ((512*((((-131584) + x3 + (787968*x2)) // 512) % 513)) + (262144*((((-131584) + x3 + (787968*x2)) // 262656) % 144)) + (x3 % 512)), tmp34, other=0.0)
    tmp36 = tl.where(tmp34, tmp35, 0.0)
    tmp37 = tl.where(tmp33, tmp36, 0.0)
    tmp38 = tl.load(in_ptr1 + (x3 + (525312*x2)), tmp17, other=0.0)
