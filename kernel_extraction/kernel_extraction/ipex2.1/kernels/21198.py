...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/bfloat16/yw/cywtvzoea7z6hqm44u5foeebe2dm4ctgipa6nzl6wpr7fibmosw3.py
...
@pointwise(size_hints=[512], filename=__file__, meta={'signature': {0: '*bf16', 1: '*bf16', 2: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_add_copy_mul_select_scatter_slice_slice_scatter_sub_zeros_like_1', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2), equal_to_1=())]})
@triton.jit
def triton_poi_fused_add_copy_mul_select_scatter_slice_slice_scatter_sub_zeros_like_1(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    x1 = (xindex // 4)
    x2 = xindex
    tmp3 = tl.load(in_ptr0 + (3 + (4*x1)), xmask, eviction_policy='evict_last').to(tl.float32)
    tmp4 = tl.load(in_ptr0 + (1 + (4*x1)), xmask, eviction_policy='evict_last').to(tl.float32)
    tmp18 = tl.load(in_ptr0 + (2 + (4*x1)), xmask, eviction_policy='evict_last').to(tl.float32)
    tmp19 = tl.load(in_ptr0 + (4*x1), xmask, eviction_policy='evict_last').to(tl.float32)
    tmp0 = x0
