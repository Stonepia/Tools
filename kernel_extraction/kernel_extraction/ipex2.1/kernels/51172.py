...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float32/os/cos3i7tnjvou3n7x5qtgr3njztczyhrnax667gwntocuarj6xxdq.py
...
@pointwise(size_hints=[33554432], filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_copy_new_zeros_select_scatter_slice_slice_scatter_4', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2), equal_to_1=())]})
@triton.jit
def triton_poi_fused_copy_new_zeros_select_scatter_slice_slice_scatter_4(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp8 = tmp6 < tmp7
    tmp9 = tmp8 & tmp5
    tmp10 = tl.load(in_ptr0 + ((512*(((3808256 + x5) // 512) % 513)) + (262144*((3808256 + x5) // 262656)) + (3932160*x3) + (3932160*((3808256 + x5) // 3939840)) + (x5 % 512)), tmp9, eviction_policy='evict_last', other=0.0)
    ...
    tmp17 = tmp16 < tmp7
    tmp18 = tmp17 & tmp15
    tmp19 = tl.load(in_ptr0 + ((262144*(((3939584 + x0 + (513*x1)) // 262656) % 15)) + (3932160*(((3939584 + x0 + (513*x1) + (3939840*x3)) // 3939840) % 12)) + ((3939584 + x0 + (513*x1)) % 262656)), tmp18, eviction_policy='evict_last', other=0.0)
    ...
    tmp30 = tmp29 < tmp7
    tmp31 = tmp30 & tmp28
    tmp32 = tl.load(in_ptr0 + ((262144*((((-256) + x0 + (513*x1) + (262656*x2) + (3939840*x3)) // 262656) % 180)) + (((-256) + x0 + (513*x1) + (262656*x2) + (3939840*x3)) % 262656)), tmp31, other=0.0)
