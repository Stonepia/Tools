...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/float32/4a/c4aqsjw7qamsluag2jwrn2lbmbdf3w7uualjpuj2glskuu32uf3m.py
...
@persistent_reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*fp32', 1: '*fp32', 2: '*fp32', 3: '*fp32', 4: '*fp32', 5: '*fp32', 6: '*fp32', 7: 'i32', 8: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_per_fused_native_group_norm_35', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5, 6, 7), equal_to_1=())]}
)
@triton.jit
def triton_per_fused_native_group_norm_35(in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr0, out_ptr1, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr):
    ...
    tmp5 = tmp3 < tmp4
    tmp6 = tmp5 & tmp2
    tmp7 = tl.load(in_ptr0 + ((384*((r3 + (104*x0) + (15053*x1)) % 196)) + (75264*x2) + (((r3 + (104*x0) + (15053*x1)) // 196) % 384)), rmask & tmp6 & xmask, other=0.0)
    tmp8 = tl.load(in_ptr1 + ((384*((r3 + (104*x0) + (15053*x1)) % 196)) + (75264*x2) + (((r3 + (104*x0) + (15053*x1)) // 196) % 384)), rmask & tmp6 & xmask, other=0.0)
    tmp9 = tl.load(in_ptr2 + ((384*((r3 + (104*x0) + (15053*x1)) % 196)) + (75264*x2) + (((r3 + (104*x0) + (15053*x1)) // 196) % 384)), rmask & tmp6 & xmask, other=0.0)
    tmp10 = tmp8 - tmp9
    tmp11 = tl.load(in_ptr3 + (((r3 + (104*x0) + (15053*x1)) // 196) % 384), rmask & tmp6 & xmask, eviction_policy='evict_last', other=0.0)
    tmp12 = tmp10 * tmp11
    tmp13 = tmp7 + tmp12
