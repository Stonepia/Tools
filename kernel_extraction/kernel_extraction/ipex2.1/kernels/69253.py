...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/amp_bf16/i7/ci7lcx3xna7wowj44wzpf5be4mh6zrfidc6m6t6vd3mk2gn4ppn2.py
# Source Nodes: [getattr_getattr_l__self___stages___0___blocks___0___token_mixer_pool], Original ATen: [aten.avg_pool2d]
# getattr_getattr_l__self___stages___0___blocks___0___token_mixer_pool => avg_pool2d
...
@pointwise(size_hints=[33554432], filename=__file__, meta={'signature': {0: '*bf16', 1: '*bf16', 2: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_avg_pool2d_6', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2), equal_to_1=())]})
@triton.jit
def triton_poi_fused_avg_pool2d_6(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp9 = tmp7 & tmp8
    tmp10 = tmp5 & tmp9
    tmp11 = tl.load(in_ptr0 + ((-5472) + x6), tmp10, other=0.0).to(tl.float32)
    ...
    tmp24 = tmp22 & tmp23
    tmp25 = tmp5 & tmp24
    tmp26 = tl.load(in_ptr0 + ((-5280) + x6), tmp25, other=0.0).to(tl.float32)
    ...
    tmp32 = tmp30 & tmp31
    tmp33 = tmp32 & tmp9
    tmp34 = tl.load(in_ptr0 + ((-96) + x6), tmp33, other=0.0).to(tl.float32)
    ...
    tmp40 = tmp39 + tmp36
    tmp41 = tmp32 & tmp24
    tmp42 = tl.load(in_ptr0 + (96 + x6), tmp41, other=0.0).to(tl.float32)
    ...
    tmp48 = tmp46 & tmp47
    tmp49 = tmp48 & tmp9
    tmp50 = tl.load(in_ptr0 + (5280 + x6), tmp49, other=0.0).to(tl.float32)
    ...
    tmp56 = tmp55 + tmp52
    tmp57 = tmp48 & tmp24
    tmp58 = tl.load(in_ptr0 + (5472 + x6), tmp57, other=0.0).to(tl.float32)
