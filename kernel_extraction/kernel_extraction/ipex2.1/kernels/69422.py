...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/bfloat16/5i/c5im4psm2tgjsn7phwtdqiunvmgyrn5gfrse6gdceeulnc6lytnd.py
...
def triton_poi_fused_add_mul_sub_53(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, out_ptr0, ynumel, xnumel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    ...
    tmp2 = tl.load(in_ptr2 + (x2 + (768*y3)), xmask & ymask, eviction_policy='evict_last').to(tl.float32)
    tmp4 = tl.load(in_ptr3 + (x2), xmask, eviction_policy='evict_last').to(tl.float32)
    tmp7 = tl.load(in_ptr4 + (x2 + (768*y3)), xmask & ymask, eviction_policy='evict_last').to(tl.float32)
