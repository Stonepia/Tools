...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/float32/zt/czt4aq7nt2bh2doahlcmjjnrrg2pvlkn3idcl3nruk2gzqszcc6h.py
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*fp32', 1: '*fp32', 2: '*fp32', 3: '*fp32', 4: 'i32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax_add_mul_24', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax_add_mul_24(in_ptr0, in_ptr1, in_ptr2, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp8 = tmp6 < tmp7
        tmp9 = tmp8 & tmp5
        tmp10 = tl.load(in_ptr1 + ((23*((11 + (23*(x0 // 4)) + (r2 // 12)) // 24)) + (92*(x0 % 4)) + (368*((((4*x1) + (x0 % 4)) // 4) % 4096)) + ((11 + (23*(x0 // 4)) + (r2 // 12)) % 24)), rmask & tmp9, eviction_policy='evict_last', other=0.0)
        ...
        tmp16 = tmp15 < tmp7
        tmp17 = tmp16 & tmp14
        tmp18 = tl.load(in_ptr2 + ((23*(((11 + (23*(x0 % 4)) + (r2 % 12)) // 24) % 4)) + (92*(x0 // 4)) + (368*x1) + ((11 + (23*(x0 % 4)) + (r2 % 12)) % 24)), rmask & tmp17, eviction_policy='evict_last', other=0.0)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp27 = 0.25
        tmp28 = tmp26 * tmp27
        tmp29 = 11 + (23*(x0 // 4)) + (r2 // 12)
        tmp30 = tl.full([1, 1], 96, tl.int64)
        tmp31 = tmp29 < tmp30
        tmp32 = (11 + (23*(x0 // 4)) + (r2 // 12)) % 24
        tmp33 = tl.full([1, 1], 23, tl.int64)
        tmp34 = tmp32 < tmp33
        tmp35 = tmp34 & tmp31
        tmp36 = tl.load(in_ptr1 + ((23*((11 + (23*(x0 // 4)) + (r2 // 12)) // 24)) + (92*(x0 % 4)) + (368*((((4*x1) + (x0 % 4)) // 4) % 4096)) + ((11 + (23*(x0 // 4)) + (r2 // 12)) % 24)), rmask & tmp35, eviction_policy='evict_last', other=0.0)
        tmp37 = tl.where(tmp35, tmp36, 0.0)
        tmp38 = tl.where(tmp31, tmp37, 0.0)
        tmp39 = 11 + (23*(x0 % 4)) + (r2 % 12)
        tmp40 = tmp39 < tmp30
        tmp41 = (11 + (23*(x0 % 4)) + (r2 % 12)) % 24
        tmp42 = tmp41 < tmp33
        tmp43 = tmp42 & tmp40
        tmp44 = tl.load(in_ptr2 + ((23*(((11 + (23*(x0 % 4)) + (r2 % 12)) // 24) % 4)) + (92*(x0 // 4)) + (368*x1) + ((11 + (23*(x0 % 4)) + (r2 % 12)) % 24)), rmask & tmp43, eviction_policy='evict_last', other=0.0)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp55 = 0.25
        tmp56 = tmp54 * tmp55
        tmp57 = 11 + (23*(x0 // 4)) + (r2 // 12)
        tmp58 = tl.full([1, 1], 96, tl.int64)
        tmp59 = tmp57 < tmp58
        tmp60 = (11 + (23*(x0 // 4)) + (r2 // 12)) % 24
        tmp61 = tl.full([1, 1], 23, tl.int64)
        tmp62 = tmp60 < tmp61
        tmp63 = tmp62 & tmp59
        tmp64 = tl.load(in_ptr1 + ((23*((11 + (23*(x0 // 4)) + (r2 // 12)) // 24)) + (92*(x0 % 4)) + (368*((((4*x1) + (x0 % 4)) // 4) % 4096)) + ((11 + (23*(x0 // 4)) + (r2 // 12)) % 24)), rmask & tmp63, eviction_policy='evict_first', other=0.0)
        tmp65 = tl.where(tmp63, tmp64, 0.0)
        tmp66 = tl.where(tmp59, tmp65, 0.0)
        tmp67 = 11 + (23*(x0 % 4)) + (r2 % 12)
        tmp68 = tmp67 < tmp58
        tmp69 = (11 + (23*(x0 % 4)) + (r2 % 12)) % 24
        tmp70 = tmp69 < tmp61
        tmp71 = tmp70 & tmp68
        tmp72 = tl.load(in_ptr2 + ((23*(((11 + (23*(x0 % 4)) + (r2 % 12)) // 24) % 4)) + (92*(x0 // 4)) + (368*x1) + ((11 + (23*(x0 % 4)) + (r2 % 12)) % 24)), rmask & tmp71, eviction_policy='evict_first', other=0.0)
