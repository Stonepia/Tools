...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/float32/b7/cb7lvgapm2xn5ya4st6kbk5w42u46nectga4r3zv5z765bhyebi3.py
...
@pointwise(size_hints=[8388608], filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: '*fp32', 3: '*fp32', 4: '*fp32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_copy_slice_scatter_10', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5), equal_to_1=())]})
@triton.jit
def triton_poi_fused_copy_slice_scatter_10(in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    x3 = (xindex // 1575936)
    x7 = xindex % 131328
    tmp7 = tl.load(in_ptr1 + (x5), None)
    tmp24 = tl.load(in_ptr3 + (x7 + (525312*x6)), None)
    tmp0 = x0
    tmp1 = tl.full([1], 257, tl.int64)
    tmp2 = tmp0 < tmp1
    tmp3 = tl.load(in_ptr0 + (x0 + (257*x4)), tmp2, other=0.0)
    ...
    tmp16 = tmp14 < tmp15
    tmp17 = tmp16 & tmp13
    tmp18 = tl.load(in_ptr2 + ((512*((((-131584) + x7 + (787968*x2) + (9455616*x3)) // 512) % 513)) + (262144*((((-131584) + x7 + (787968*x2) + (9455616*x3)) // 262656) % 144)) + (x7 % 512)), tmp17, other=0.0)
    tmp19 = tl.where(tmp17, tmp18, 0.0)
    tmp20 = tl.where(tmp13, tmp19, 0.0)
    tmp21 = tl.load(in_ptr3 + (x7 + (525312*x6)), tmp10, other=0.0)
