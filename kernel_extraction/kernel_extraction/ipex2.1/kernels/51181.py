...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float32/sc/cscxfo7wxzysw5q65cw2zj7zsin72vdrkzufddrhqtzeofozfg5h.py
...
def triton_poi_fused__to_copy_full_like_where_13(in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr0, ynumel, xnumel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    ...
    tmp28 = tl.load(in_ptr3 + (y0 + (4096*x1)), xmask & ymask, eviction_policy='evict_last')
    tmp0 = (-255) + x1 + y0
    ...
    tmp18 = tmp16 < tmp17
    tmp19 = tmp18 & tmp15
    tmp20 = tl.load(in_ptr1 + ((256*((((-131584) + x1 + (513*y0)) // 262656) % 15)) + ((((-131584) + x1 + (513*y0)) // 512) % 513)), tmp19 & xmask & ymask, eviction_policy='evict_last', other=0.0)
    tmp21 = tl.load(in_ptr2 + ((256*((((-131584) + x1 + (513*y0)) // 262656) % 15)) + ((x1 + (513*y0)) % 512)), tmp19 & xmask & ymask, eviction_policy='evict_last', other=0.0)
    tmp22 = tmp20 * tmp21
    tmp23 = tl.where(tmp19, tmp22, 0.0)
    tmp24 = tl.where(tmp15, tmp23, 0.0)
    tmp25 = tl.load(in_ptr3 + (y0 + (4096*x1)), tmp11 & xmask & ymask, eviction_policy='evict_last', other=0.0)
