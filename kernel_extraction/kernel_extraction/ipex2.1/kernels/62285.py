...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/float32/7h/c7h2bjiov64stvcnesgqig4ht22cbnvx6x35ntc6a33oz6n7xqbo.py
# Source Nodes: [dropout_6, softmax_6], Original ATen: [aten._softmax, aten.native_dropout]
...
def triton_red_fused__softmax_native_dropout_11(in_ptr0, in_ptr1, in_ptr2, out_ptr1, out_ptr3, out_ptr4, out_ptr5, out_ptr6, load_seed_offset, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r3 = rindex
        tmp33 = tl.load(in_ptr0 + (r3 + (1024*x4)), rmask, eviction_policy='evict_first', other=0.0)
        ...
        tmp52 = tl.load(in_ptr1 + (x1 + (8*tmp51)), None, eviction_policy='evict_first')
        tmp53 = r3
        tmp54 = x0
        tmp55 = tmp53 <= tmp54
        ...
        tl.store(out_ptr1 + (r3 + (1024*x4)), tmp63, rmask)
        tl.store(out_ptr3 + (r3 + (1024*x4)), tmp70, rmask)
    tmp66 = tl.sum(_tmp66, 1)[:, None]
    for roffset in range(0, rnumel, RBLOCK):
        rindex = roffset + rbase
        rmask = rindex < rnumel
        r3 = rindex
        tmp71 = tl.load(out_ptr3 + (r3 + (1024*x4)), rmask, eviction_policy='evict_first', other=0.0)
        tmp74 = tl.load(out_ptr1 + (r3 + (1024*x4)), rmask, eviction_policy='evict_first', other=0.0)
        tmp72 = 0.1
        tmp73 = tmp71 > tmp72
        tmp75 = tl.exp(tmp74)
        tmp76 = tmp75 / tmp66
        ...
        tmp80 = tmp78 * tmp79
        tl.store(out_ptr4 + (r3 + (1024*x4)), tmp73, rmask)
        tl.store(out_ptr5 + (r3 + (1024*x4)), tmp76, rmask)
        tl.store(out_ptr6 + (r3 + (1024*x4)), tmp80, rmask)
''')
