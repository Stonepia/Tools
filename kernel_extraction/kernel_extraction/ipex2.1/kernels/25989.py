...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float32/q6/cq6jqzujpiuijhspiey36bpnmgmhstz4yxivbt5jdmjlpwwchxai.py
...
def triton_red_fused__softmax_5(in_ptr0, in_ptr1, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp55 = tl.load(in_ptr1 + (x1 + (8*tmp54)), rmask, eviction_policy='evict_last')
        tmp56 = tmp30 + tmp55
        tmp57 = tmp56 - tmp28
        tmp58 = tl.exp(tmp57)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp87 = tl.load(in_ptr1 + (x1 + (8*tmp86)), rmask, eviction_policy='evict_first')
        tmp88 = tmp62 + tmp87
        tmp89 = tmp88 - tmp28
        tmp90 = tl.exp(tmp89)
        tmp91 = tmp90 / tmp60
