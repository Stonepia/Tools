...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float32/jk/cjkwshcb2luspyd2ygt4ytpbxuorcbiyxsyzp6yfm7ltgsa5tn3o.py
...
# nan_to_num_2 => eq_10, eq_11, isnan_2, where_7, where_8, where_9
# softmax_2 => amax_2, div_2, exp_2, sub_8, sum_3
# triu => full_default_1
triton_red_fused__softmax_add_nan_to_num_triu_17 = async_compile.triton('triton_red_fused__softmax_add_nan_to_num_triu_17', '''
import triton
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*fp32', 1: '*fp32', 2: '*fp32', 3: 'i32', 4: 'i32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax_add_nan_to_num_triu_17', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 4), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax_add_nan_to_num_triu_17(in_ptr0, in_ptr1, out_ptr2, ks0, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 16
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp0 = tl.load(in_ptr0 + (r1 + (3*x0) + (ks0*x0)), rmask & xmask, eviction_policy='evict_last', other=0.0)
        tmp1 = float("inf")
        tmp2 = tmp0 == tmp1
        tmp3 = float("-inf")
        tmp4 = tmp0 == tmp3
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp18 = tl.load(in_ptr0 + (r1 + (3*x0) + (ks0*x0)), rmask & xmask, eviction_policy='evict_last', other=0.0)
        tmp19 = float("inf")
        tmp20 = tmp18 == tmp19
        tmp21 = float("-inf")
        tmp22 = tmp18 == tmp21
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp38 = tl.load(in_ptr0 + (r1 + (3*x0) + (ks0*x0)), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp39 = float("inf")
        tmp40 = tmp38 == tmp39
        tmp41 = float("-inf")
        tmp42 = tmp38 == tmp41
        ...
        tmp54 = tl.exp(tmp53)
        tmp55 = tmp54 / tmp36
        tl.store(out_ptr2 + (r1 + (3*x0) + (ks0*x0)), tmp55, rmask & xmask)
''')
