...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float16/5z/c5zwcpejoms3nc7t6nustsvqn34lcftcevg2wqxffoybonoe2a3z.py
# Source Nodes: [add, l__mod___transformer_wpe, l__mod___transformer_wte, layer_norm], Original ATen: [aten.add, aten.embedding, aten.native_layer_norm]
# add => add
# l__mod___transformer_wpe => embedding_1
# l__mod___transformer_wte => embedding
# layer_norm => add_1, add_2, convert_element_type, convert_element_type_1, mul, mul_1, rsqrt, sub, var_mean
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*i64', 1: '*fp16', 2: '*fp16', 3: '*fp16', 4: '*fp16', 5: '*fp16', 6: 'i32', 7: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused_add_embedding_native_layer_norm_0', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5, 6, 7), equal_to_1=())]}
)
@triton.jit
def triton_red_fused_add_embedding_native_layer_norm_0(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp3 = tl.load(in_ptr2 + (r1 + (768*x0)), rmask & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp12 = tl.load(in_ptr2 + (r1 + (768*x0)), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        ...
        tmp10 = tl.where(tmp0 < 0, tmp0 + 50304, tmp0)
        # tl.device_assert(((0 <= tmp10) & (tmp10 < 50304)) | ~xmask, "index out of bounds: 0 <= tmp10 < 50304")
        tmp11 = tl.load(in_ptr1 + (r1 + (768*tmp10)), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp13 = tmp11 + tmp12
        ...
        tl.store(out_ptr2 + (r1 + (768*x0)), tmp28, rmask & xmask)
''')
