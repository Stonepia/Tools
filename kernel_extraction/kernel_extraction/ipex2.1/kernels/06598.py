...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float16/hh/chhk5smrg3lukwwagkeidlkw5waim223fsgvhm5h7huepgzrevh6.py
# Source Nodes: [add, add_56, add_57, clamp_24, l__mod___model_decoder_block_0_layer_0_dropout, l__mod___model_encoder_embed_tokens, l__mod___model_encoder_embed_tokens_1, mean, mean_26, mul_1, mul_2, mul_59, mul_60, neg_25, pow_1, pow_27, rsqrt, rsqrt_26, to_1, to_2, to_59, to_60, where_1, where_26], Original ATen: [aten._to_copy, aten.add, aten.clamp, aten.clone, aten.embedding, aten.mean, aten.mul, aten.neg, aten.pow, aten.rsqrt, aten.scalar_tensor, aten.where]
# add => add
# add_56 => add_70
# add_57 => add_71
# clamp_24 => clamp_max_24, clamp_min_24, convert_element_type_135, convert_element_type_136
...
# to_59 => convert_element_type_137
# to_60 => convert_element_type_138
...
def triton_red_fused__to_copy_add_clamp_clone_embedding_mean_mul_neg_pow_rsqrt_scalar_tensor_where_6(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, out_ptr2, out_ptr3, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp22 = tl.load(in_ptr1 + (r1 + (768*tmp21)), rmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp31 = tl.load(in_ptr2 + (r1 + (768*x0)), rmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp29 = tl.where(tmp0 < 0, tmp0 + 32128, tmp0)
        # tl.device_assert((0 <= tmp29) & (tmp29 < 32128), "index out of bounds: 0 <= tmp29 < 32128")
        tmp30 = tl.load(in_ptr1 + (r1 + (768*tmp29)), rmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp32 = tmp30 + tmp31
        ...
        tmp53 = tl.load(in_ptr1 + (r1 + (768*tmp52)), rmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        ...
        tmp60 = tmp28 * tmp59
        tl.store(out_ptr2 + (r1 + (768*x0)), tmp51, rmask)
        tl.store(out_ptr3 + (r1 + (768*x0)), tmp60, rmask)
''')
