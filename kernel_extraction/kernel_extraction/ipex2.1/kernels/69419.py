...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/bfloat16/sx/csxyabryjtrc653a3qjsagyeeuftqujb6xoro67rpj6dev45fovz.py
# Source Nodes: [group_norm_61], Original ATen: [aten.native_group_norm]
# group_norm_61 => convert_element_type_304, var_mean_61
...
@persistent_reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*bf16', 1: '*bf16', 2: '*bf16', 3: '*bf16', 4: '*fp32', 5: '*fp32', 6: '*fp32', 7: 'i32', 8: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_per_fused_native_group_norm_50', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5, 6, 7, 8), equal_to_1=())]}
)
@triton.jit
def triton_per_fused_native_group_norm_50(in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr0, out_ptr1, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr):
    ...
    tmp8 = tl.load(in_ptr1 + ((768*((r3 + (128*x0) + (7527*x1)) % 49)) + (37632*x2) + (((r3 + (128*x0) + (7527*x1)) // 49) % 768)), rmask & tmp6 & xmask, other=0.0).to(tl.float32)
    tmp9 = tl.load(in_ptr2 + ((768*((r3 + (128*x0) + (7527*x1)) % 49)) + (37632*x2) + (((r3 + (128*x0) + (7527*x1)) // 49) % 768)), rmask & tmp6 & xmask, other=0.0).to(tl.float32)
    tmp10 = tmp8 - tmp9
    tmp11 = tl.load(in_ptr3 + (((r3 + (128*x0) + (7527*x1)) // 49) % 768), rmask & tmp6 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    tmp12 = tmp10 * tmp11
    tmp13 = tmp7 + tmp12
