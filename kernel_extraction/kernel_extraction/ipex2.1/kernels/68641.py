...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/bfloat16/24/c24un5o3wmj3bfcz6o4brg7l64ooxqwbpp6idizmwfktrn3n35jr.py
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*bf16', 1: '*bf16', 2: '*bf16', 3: 'i32', 4: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax__to_copy_5', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax__to_copy_5(in_ptr0, in_ptr1, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp25 = tl.load(in_ptr1 + (x1 + (16*tmp24)), rmask, eviction_policy='evict_last').to(tl.float32)
        tmp26 = tmp0 + tmp25
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r2 = rindex
        tmp31 = tl.load(in_ptr0 + (r2 + (512*x3)), rmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        tmp32 = r2 + ((-1)*x0)
        ...
        tmp56 = tl.load(in_ptr1 + (x1 + (16*tmp55)), rmask, eviction_policy='evict_last').to(tl.float32)
        tmp57 = tmp31 + tmp56
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r2 = rindex
        tmp64 = tl.load(in_ptr0 + (r2 + (512*x3)), rmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp65 = r2 + ((-1)*x0)
        ...
        tmp89 = tl.load(in_ptr1 + (x1 + (16*tmp88)), rmask, eviction_policy='evict_first').to(tl.float32)
        tmp90 = tmp64 + tmp89
        ...
        tl.store(out_ptr2 + (r2 + (512*x3)), tmp95, rmask)
''')
