...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float32/cz/cczjwkng7hxrox3q5njegsggrosqgb7r3dwricg5m2el3zc6azmi.py
...
def triton_poi_fused_max_pool2d_with_indices_1(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp9 = tmp7 & tmp8
    tmp10 = tmp5 & tmp9
    tmp11 = tl.load(in_ptr0 + ((-32832) + x0 + (128*x1) + (65536*x2)), tmp10, other=0.0)
    ...
    tmp16 = tmp14 & tmp15
    tmp17 = tmp5 & tmp16
    tmp18 = tl.load(in_ptr0 + ((-32768) + x0 + (128*x1) + (65536*x2)), tmp17, other=0.0)
    ...
    tmp24 = tmp22 & tmp23
    tmp25 = tmp5 & tmp24
    tmp26 = tl.load(in_ptr0 + ((-32704) + x0 + (128*x1) + (65536*x2)), tmp25, other=0.0)
    ...
    tmp32 = tmp30 & tmp31
    tmp33 = tmp32 & tmp9
    tmp34 = tl.load(in_ptr0 + ((-64) + x0 + (128*x1) + (65536*x2)), tmp33, other=0.0)
    tmp35 = tl.where(tmp33, tmp34, float("-inf"))
    tmp36 = triton_helpers.maximum(tmp35, tmp28)
    tmp37 = tmp32 & tmp16
    tmp38 = tl.load(in_ptr0 + (x0 + (128*x1) + (65536*x2)), tmp37, other=0.0)
    tmp39 = tl.where(tmp37, tmp38, float("-inf"))
    tmp40 = triton_helpers.maximum(tmp39, tmp36)
    tmp41 = tmp32 & tmp24
    tmp42 = tl.load(in_ptr0 + (64 + x0 + (128*x1) + (65536*x2)), tmp41, other=0.0)
    ...
    tmp48 = tmp46 & tmp47
    tmp49 = tmp48 & tmp9
    tmp50 = tl.load(in_ptr0 + (32704 + x0 + (128*x1) + (65536*x2)), tmp49, other=0.0)
    tmp51 = tl.where(tmp49, tmp50, float("-inf"))
    tmp52 = triton_helpers.maximum(tmp51, tmp44)
    tmp53 = tmp48 & tmp16
    tmp54 = tl.load(in_ptr0 + (32768 + x0 + (128*x1) + (65536*x2)), tmp53, other=0.0)
    tmp55 = tl.where(tmp53, tmp54, float("-inf"))
    tmp56 = triton_helpers.maximum(tmp55, tmp52)
    tmp57 = tmp48 & tmp24
    tmp58 = tl.load(in_ptr0 + (32832 + x0 + (128*x1) + (65536*x2)), tmp57, other=0.0)
