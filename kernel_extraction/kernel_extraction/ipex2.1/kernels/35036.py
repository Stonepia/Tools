...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/amp_fp16/bz/cbzodxerjz3tu7xyqadx6wmrzxvxnarc6dy2b53fti6do4bsrlfu.py
...
def triton_poi_fused_copy_div_mul_slice_scatter_sub_zeros_like_0(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, out_ptr0, out_ptr1, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp4 = tl.load(in_ptr1 + (15912 + (3*x3)), tmp2 & xmask, other=0.0)
    tmp5 = tl.load(in_ptr1 + (3*x3), tmp2 & xmask, other=0.0)
    tmp6 = tmp4 - tmp5
    tmp7 = tmp3 * tmp6
    tmp8 = tl.load(in_ptr2 + (x2), tmp2 & xmask, eviction_policy='evict_last', other=0.0)
    tmp9 = tl.load(in_ptr3 + (x1), tmp2 & xmask, eviction_policy='evict_last', other=0.0)
    tmp10 = tmp8 * tmp9
    tmp11 = tmp7 / tmp10
    tmp12 = tl.where(tmp2, tmp11, 0.0)
    tmp13 = tl.full([1], 0.0, tl.float64)
    tmp14 = tl.where(tmp2, tmp12, tmp13)
    tmp15 = tl.load(in_ptr4 + (15912 + (3*x3)), tmp2 & xmask, other=0.0)
    tmp16 = tl.load(in_ptr4 + (3*x3), tmp2 & xmask, other=0.0)
    tmp17 = tmp15 - tmp16
    tmp18 = tmp3 * tmp17
    tmp19 = tmp18 / tmp10
