...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float32/yz/cyzyg3vnenwlpvsgoy2fzxfbvxgb35u7d2ks6kwjtr5vfnd4546f.py
...
# nan_to_num_5 => eq_10, eq_11, isnan_5, where_16, where_17, where_18
# softmax_5 => amax_5, exp_5, sub_17, sum_6
# triu => full_default_1
triton_poi_fused__softmax_add_nan_to_num_triu_29 = async_compile.triton('triton_poi_fused__softmax_add_nan_to_num_triu_29', '''
import triton
...
@pointwise(size_hints=[16], filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: '*fp32', 3: '*fp32', 4: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused__softmax_add_nan_to_num_triu_29', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4), equal_to_1=())]})
@triton.jit
def triton_poi_fused__softmax_add_nan_to_num_triu_29(in_ptr0, in_ptr1, out_ptr0, out_ptr1, xnumel, XBLOCK : tl.constexpr):
    xnumel = 16
    xoffset = tl.program_id(0) * XBLOCK
    xindex = xoffset + tl.arange(0, XBLOCK)[:]
    xmask = xindex < xnumel
    x0 = xindex
    tmp0 = tl.load(in_ptr0 + (6*x0), xmask)
    tmp12 = tl.load(in_ptr1 + (0))
    tmp13 = tl.broadcast_to(tmp12, [XBLOCK])
    tmp15 = tl.load(in_ptr0 + (1 + (6*x0)), xmask)
    tmp24 = tl.load(in_ptr0 + (2 + (6*x0)), xmask)
    tmp33 = tl.load(in_ptr0 + (3 + (6*x0)), xmask)
    tmp42 = tl.load(in_ptr0 + (4 + (6*x0)), xmask)
    tmp51 = tl.load(in_ptr0 + (5 + (6*x0)), xmask)
    tmp1 = float("inf")
    tmp2 = tmp0 == tmp1
    tmp3 = float("-inf")
    tmp4 = tmp0 == tmp3
