...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/bfloat16/ki/ckipwyjou4gpyr3un7f32s22cqheojrgzvijbycvrubxykivyx4f.py
# Source Nodes: [add_6, mul, softmax], Original ATen: [aten._softmax, aten.add, aten.mul]
# add_6 => add_50
# mul => mul_66
# softmax => amax, convert_element_type_66, convert_element_type_67, div, exp, sub_22, sum_1
triton_red_fused__softmax_add_mul_6 = async_compile.triton('triton_red_fused__softmax_add_mul_6', '''
import triton
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*bf16', 1: '*bf16', 2: '*bf16', 3: '*bf16', 4: 'i32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax_add_mul_6', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax_add_mul_6(in_ptr0, in_ptr1, in_ptr2, out_ptr3, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 131072
    rnumel = 256
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp8 = tmp6 < tmp7
        tmp9 = tmp8 & tmp5
        tmp10 = tl.load(in_ptr1 + ((31*((15 + (31*(x0 // 16)) + (r2 // 16)) // 32)) + (496*(x0 % 16)) + (7936*((((16*x1) + (x0 % 16)) // 16) % 512)) + ((15 + (31*(x0 // 16)) + (r2 // 16)) % 32)), rmask & tmp9, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp35 = tmp33 < tmp34
        tmp36 = tmp35 & tmp32
        tmp37 = tl.load(in_ptr1 + ((31*((15 + (31*(x0 // 16)) + (r2 // 16)) // 32)) + (496*(x0 % 16)) + (7936*((((16*x1) + (x0 % 16)) // 16) % 512)) + ((15 + (31*(x0 // 16)) + (r2 // 16)) % 32)), rmask & tmp36, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp64 = tmp62 < tmp63
        tmp65 = tmp64 & tmp61
        tmp66 = tl.load(in_ptr1 + ((31*((15 + (31*(x0 // 16)) + (r2 // 16)) // 32)) + (496*(x0 % 16)) + (7936*((((16*x1) + (x0 % 16)) // 16) % 512)) + ((15 + (31*(x0 // 16)) + (r2 // 16)) % 32)), rmask & tmp65, eviction_policy='evict_first', other=0.0).to(tl.float32)
        ...
        tl.store(out_ptr3 + (r2 + (256*x3)), tmp83, rmask)
''')
