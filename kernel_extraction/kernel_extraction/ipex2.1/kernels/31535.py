...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/amp_bf16/re/cre4himfvwjmagkp4gcygshrb5ogiwikjwargaikrfbqox4h4he7.py
# Source Nodes: [getattr_l__self___layer3___0___avd_last], Original ATen: [aten.avg_pool2d]
# getattr_l__self___layer3___0___avd_last => avg_pool2d_2
...
@pointwise(size_hints=[2097152], filename=__file__, meta={'signature': {0: '*bf16', 1: '*bf16', 2: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_avg_pool2d_11', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2), equal_to_1=())]})
@triton.jit
def triton_poi_fused_avg_pool2d_11(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp9 = tmp7 & tmp8
    tmp10 = tmp5 & tmp9
    tmp11 = tl.load(in_ptr0 + ((-7424) + x0 + (512*x1) + (14336*x5)), tmp10, other=0.0).to(tl.float32)
    ...
    tmp16 = tmp14 & tmp15
    tmp17 = tmp5 & tmp16
    tmp18 = tl.load(in_ptr0 + ((-7168) + x0 + (512*x1) + (14336*x5)), tmp17, other=0.0).to(tl.float32)
    ...
    tmp24 = tmp22 & tmp23
    tmp25 = tmp5 & tmp24
    tmp26 = tl.load(in_ptr0 + ((-6912) + x0 + (512*x1) + (14336*x5)), tmp25, other=0.0).to(tl.float32)
    ...
    tmp32 = tmp30 & tmp31
    tmp33 = tmp32 & tmp9
    tmp34 = tl.load(in_ptr0 + ((-256) + x0 + (512*x1) + (14336*x5)), tmp33, other=0.0).to(tl.float32)
    tmp35 = tl.where(tmp33, tmp34, 0.0)
    tmp36 = tmp35 + tmp28
    tmp37 = tmp32 & tmp16
    tmp38 = tl.load(in_ptr0 + (x0 + (512*x1) + (14336*x5)), tmp37, other=0.0).to(tl.float32)
    tmp39 = tl.where(tmp37, tmp38, 0.0)
    tmp40 = tmp39 + tmp36
    tmp41 = tmp32 & tmp24
    tmp42 = tl.load(in_ptr0 + (256 + x0 + (512*x1) + (14336*x5)), tmp41, other=0.0).to(tl.float32)
    ...
    tmp48 = tmp46 & tmp47
    tmp49 = tmp48 & tmp9
    tmp50 = tl.load(in_ptr0 + (6912 + x0 + (512*x1) + (14336*x5)), tmp49, other=0.0).to(tl.float32)
    tmp51 = tl.where(tmp49, tmp50, 0.0)
    tmp52 = tmp51 + tmp44
    tmp53 = tmp48 & tmp16
    tmp54 = tl.load(in_ptr0 + (7168 + x0 + (512*x1) + (14336*x5)), tmp53, other=0.0).to(tl.float32)
    tmp55 = tl.where(tmp53, tmp54, 0.0)
    tmp56 = tmp55 + tmp52
    tmp57 = tmp48 & tmp24
    tmp58 = tl.load(in_ptr0 + (7424 + x0 + (512*x1) + (14336*x5)), tmp57, other=0.0).to(tl.float32)
