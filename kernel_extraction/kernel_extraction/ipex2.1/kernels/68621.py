...
def triton_red_fused_add_clone_embedding_mean_mul_pow_rsqrt_4(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, out_ptr2, out_ptr3, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp3 = tl.load(in_ptr2 + (r1 + (1024*x0)), rmask & xmask, eviction_policy='evict_last', other=0.0)
        ...
        # tl.device_assert(((0 <= tmp10) & (tmp10 < 32128)) | ~xmask, "index out of bounds: 0 <= tmp10 < 32128")
        tmp11 = tl.load(in_ptr1 + (r1 + (1024*tmp10)), rmask & xmask, eviction_policy='evict_last', other=0.0)
        tmp12 = tmp11 * tmp11
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        # tl.device_assert(((0 <= tmp17) & (tmp17 < 32128)) | ~xmask, "index out of bounds: 0 <= tmp17 < 32128")
        tmp18 = tl.load(in_ptr1 + (r1 + (1024*tmp17)), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp20 = tmp18 + tmp19
        tmp21 = 1024.0
        tmp22 = tmp7 / tmp21
        tmp23 = 1e-06
        ...
        # tl.device_assert(((0 <= tmp28) & (tmp28 < 32128)) | ~xmask, "index out of bounds: 0 <= tmp28 < 32128")
