...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/amp_bf16/hr/chritx2ka77m25pyxiy6fn3ckizqmpz56tklbzncm5pr4pmltm2a.py
...
# truediv_64 => div_61
# zeros_like => full
triton_poi_fused_add_copy_div_mul_select_scatter_slice_slice_scatter_sub_zeros_like_78 = async_compile.triton('triton_poi_fused_add_copy_div_mul_select_scatter_slice_slice_scatter_sub_zeros_like_78', '''
import triton
...
@pointwise(size_hints=[2097152], filename=__file__, meta={'signature': {0: '*fp64', 1: '*fp64', 2: '*fp64', 3: '*fp64', 4: '*fp64', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_add_copy_div_mul_select_scatter_slice_slice_scatter_sub_zeros_like_78', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5), equal_to_1=())]})
@triton.jit
def triton_poi_fused_add_copy_div_mul_select_scatter_slice_slice_scatter_sub_zeros_like_78(in_ptr0, in_ptr1, in_ptr2, in_ptr3, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    xnumel = 1082016
    ...
    tmp10 = tmp8 & tmp9
    tmp11 = tmp10 & tmp5
    tmp12 = tl.load(in_ptr0 + (x1), tmp11 & xmask, eviction_policy='evict_last', other=0.0)
    tmp13 = tl.load(in_ptr1 + (3*x3), tmp11 & xmask, other=0.0)
    tmp14 = tmp12 * tmp13
    tmp15 = tl.load(in_ptr2 + (78 + (3*x3)), tmp11 & xmask, other=0.0)
    tmp16 = tl.load(in_ptr2 + (3*x3), tmp11 & xmask, other=0.0)
    tmp17 = tmp15 + tmp16
    tmp18 = tmp14 * tmp17
    tmp19 = tl.full([1], 0.5, tl.float64)
    tmp20 = tmp18 * tmp19
    tmp21 = tl.load(in_ptr3 + ((-10478) + x4 + (5226*x2)), tmp11 & xmask, other=0.0)
    tmp22 = tmp21 * tmp19
    tmp23 = tmp20 - tmp22
    tmp24 = tl.where(tmp11, tmp23, 0.0)
    tmp25 = tl.full([1], 0.0, tl.float64)
