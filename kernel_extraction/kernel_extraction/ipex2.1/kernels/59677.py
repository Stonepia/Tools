...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/float32/uv/cuvvlsber6drhuca4pl6jehixsuo773ontxqvi4you4yd55efvh4.py
...
def triton_red_fused__log_softmax_11(in_ptr0, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp0 = tl.load(in_ptr0 + (r1 + (50265*(x0 % 511)) + (25735680*(x0 // 511))), rmask & xmask, eviction_policy='evict_last', other=0.0)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp4 = tl.load(in_ptr0 + (r1 + (50265*(x0 % 511)) + (25735680*(x0 // 511))), rmask & xmask, eviction_policy='evict_last', other=0.0)
        tmp5 = tmp4 - tmp2
        tmp6 = tl.exp(tmp5)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp10 = tl.load(in_ptr0 + (r1 + (50265*(x0 % 511)) + (25735680*(x0 // 511))), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp11 = tmp10 - tmp2
        tmp12 = tl.log(tmp8)
        tmp13 = tmp11 - tmp12
