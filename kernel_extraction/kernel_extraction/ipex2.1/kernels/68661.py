...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float16/dy/cdyotvjljjueftookbacyzoitcjzx33ohr7vvhe53ltchud45qur.py
# Source Nodes: [add, add_104, add_105, clamp_48, l__mod___model_decoder_block_0_layer_0_dropout, l__mod___model_encoder_embed_tokens, l__mod___model_encoder_embed_tokens_1, mean, mean_50, mul_1, mul_107, mul_108, mul_2, neg_49, pow_1, pow_51, rsqrt, rsqrt_50, to_1, to_107, to_108, to_2, where_1, where_50], Original ATen: [aten._to_copy, aten.add, aten.clamp, aten.clone, aten.embedding, aten.mean, aten.mul, aten.neg, aten.pow, aten.rsqrt, aten.scalar_tensor, aten.where]
# add => add
# add_104 => add_130
# add_105 => add_131
# clamp_48 => clamp_max_48, clamp_min_48, convert_element_type_255, convert_element_type_256
...
# to_107 => convert_element_type_257
# to_108 => convert_element_type_258
...
def triton_red_fused__to_copy_add_clamp_clone_embedding_mean_mul_neg_pow_rsqrt_scalar_tensor_where_6(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, out_ptr2, out_ptr3, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        # tl.device_assert(((0 <= tmp21) & (tmp21 < 32128)) | ~xmask, "index out of bounds: 0 <= tmp21 < 32128")
        tmp22 = tl.load(in_ptr1 + (r1 + (1024*tmp21)), rmask & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp31 = tl.load(in_ptr2 + (r1 + (1024*x0)), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp29 = tl.where(tmp0 < 0, tmp0 + 32128, tmp0)
        # tl.device_assert(((0 <= tmp29) & (tmp29 < 32128)) | ~xmask, "index out of bounds: 0 <= tmp29 < 32128")
        tmp30 = tl.load(in_ptr1 + (r1 + (1024*tmp29)), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp32 = tmp30 + tmp31
        ...
        # tl.device_assert(((0 <= tmp52) & (tmp52 < 32128)) | ~xmask, "index out of bounds: 0 <= tmp52 < 32128")
        tmp53 = tl.load(in_ptr1 + (r1 + (1024*tmp52)), rmask & xmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        ...
        tmp60 = tmp28 * tmp59
        tl.store(out_ptr2 + (r1 + (1024*x0)), tmp51, rmask & xmask)
        tl.store(out_ptr3 + (r1 + (1024*x0)), tmp60, rmask & xmask)
''')
