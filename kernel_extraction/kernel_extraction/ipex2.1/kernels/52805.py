...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/amp_fp16/zk/czk4e2ghxeqm4oa5vz5t5ojvokwinz5ltpprbyxropagpfe5qyux.py
...
def triton_poi_fused_add_slice_backward_3(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, in_ptr6, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp3 = tmp1 < tmp2
    tmp4 = tl.load(in_ptr1 + (x4 + (25685415*x2)), tmp3, other=0.0)
    tmp5 = tl.load(in_ptr2 + (x1 + (511*x2)), tmp3, eviction_policy='evict_last')
    tmp10 = tmp7 / tmp9
    tmp11 = 0.0
    tmp12 = tl.where(tmp5, tmp10, tmp11)
    tmp13 = tmp4 * tmp12
    tmp14 = tl.load(in_ptr5 + (x4 + (25685415*x2)), tmp3, other=0.0)
    tmp15 = tl.exp(tmp14)
