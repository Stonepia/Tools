...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/float32/uh/cuhtg6qfu6ee6fw6wey7oe3nbyhttyck5fb5r6kl7pd637xmumwm.py
...
def triton_red_fused__log_softmax_10(in_ptr0, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp0 = tl.load(in_ptr0 + (r1 + (30522*(x0 % 511)) + (15627264*(x0 // 511))), rmask & xmask, eviction_policy='evict_last', other=0.0)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp4 = tl.load(in_ptr0 + (r1 + (30522*(x0 % 511)) + (15627264*(x0 // 511))), rmask & xmask, eviction_policy='evict_last', other=0.0)
        tmp5 = tmp4 - tmp2
        tmp6 = tl.exp(tmp5)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp10 = tl.load(in_ptr0 + (r1 + (30522*(x0 % 511)) + (15627264*(x0 // 511))), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp11 = tmp10 - tmp2
        tmp12 = tl.log(tmp8)
        tmp13 = tmp11 - tmp12
