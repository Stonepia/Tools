...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/bfloat16/pm/cpm53csz5rusyg5zvpmdc7n7oeqdluwawhcuyebrlb6zxduil76j.py
...
triton_red_fused__softmax_add_mul_47 = async_compile.triton('triton_red_fused__softmax_add_mul_47', '''
import triton
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*bf16', 1: '*bf16', 2: '*bf16', 3: '*bf16', 4: 'i32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax_add_mul_47', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax_add_mul_47(in_ptr0, in_ptr1, in_ptr2, out_ptr3, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 65536
    rnumel = 144
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp8 = tmp6 < tmp7
        tmp9 = tmp8 & tmp5
        tmp10 = tl.load(in_ptr1 + ((23*((11 + (23*(x0 // 8)) + (r2 // 12)) // 24)) + (184*(x0 % 8)) + (1472*((((8*x1) + (x0 % 8)) // 8) % 1024)) + ((11 + (23*(x0 // 8)) + (r2 // 12)) % 24)), rmask & tmp9, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp35 = tmp33 < tmp34
        tmp36 = tmp35 & tmp32
        tmp37 = tl.load(in_ptr1 + ((23*((11 + (23*(x0 // 8)) + (r2 // 12)) // 24)) + (184*(x0 % 8)) + (1472*((((8*x1) + (x0 % 8)) // 8) % 1024)) + ((11 + (23*(x0 // 8)) + (r2 // 12)) % 24)), rmask & tmp36, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp64 = tmp62 < tmp63
        tmp65 = tmp64 & tmp61
        tmp66 = tl.load(in_ptr1 + ((23*((11 + (23*(x0 // 8)) + (r2 // 12)) // 24)) + (184*(x0 % 8)) + (1472*((((8*x1) + (x0 % 8)) // 8) % 1024)) + ((11 + (23*(x0 // 8)) + (r2 // 12)) % 24)), rmask & tmp65, eviction_policy='evict_first', other=0.0).to(tl.float32)
