...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/float32/kw/ckwzhe6ziub3fjuosz7kpjcycjaer2v35v42z4kgrdyokla3bzgk.py
...
def triton_poi_fused_copy_slice_scatter_15(in_ptr0, in_ptr1, in_ptr2, out_ptr0, ynumel, xnumel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    ...
    y2 = (yindex // 768)
    y5 = yindex
    tmp15 = tl.load(in_ptr2 + (x3 + (513*y0) + (131328*y2)), xmask, eviction_policy='evict_last')
    tmp0 = x3
    ...
    tmp5 = tmp3 < tmp4
    tmp6 = tmp5 & tmp2
    tmp7 = tl.load(in_ptr0 + (256 + (256*y1) + (256*((131072 + x3 + (513*y0)) // 262656)) + (1024*y2) + (1024*((131072 + x3 + (513*y0) + (262656*y1)) // 787968)) + ((x3 + (513*y0)) // 512)), tmp6 & xmask, eviction_policy='evict_last', other=0.0)
    tmp8 = tl.load(in_ptr1 + ((256*y1) + (256*((131072 + x3 + (513*y0)) // 262656)) + (1024*y2) + (1024*((131072 + x3 + (513*y0) + (262656*y1)) // 787968)) + ((x3 + (513*y0)) % 512)), tmp6 & xmask, eviction_policy='evict_last', other=0.0)
    tmp9 = tmp7 * tmp8
    ...
    tmp22 = tmp21 < tmp4
    tmp23 = tmp22 & tmp20
    tmp24 = tl.load(in_ptr0 + ((256*(((262400 + x3 + (513*y0) + (262656*y1)) // 262656) % 3)) + (1024*(((262400 + x3 + (513*y0) + (262656*y1) + (787968*y2)) // 787968) % 4)) + (((262400 + x3 + (513*y0)) // 512) % 513)), tmp23 & xmask, eviction_policy='evict_last', other=0.0)
    tmp25 = tl.load(in_ptr1 + ((256*(((262400 + x3 + (513*y0) + (262656*y1)) // 262656) % 3)) + (1024*(((262400 + x3 + (513*y0) + (262656*y1) + (787968*y2)) // 787968) % 4)) + ((262400 + x3 + (513*y0)) % 512)), tmp23 & xmask, eviction_policy='evict_last', other=0.0)
    tmp26 = tmp24 * tmp25
