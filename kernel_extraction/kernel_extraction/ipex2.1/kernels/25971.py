...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/bfloat16/qs/cqsoxka3tobbwg3h34hye24joumd4aelueq5btbu4ratmrtghhco.py
...
def triton_red_fused__softmax__to_copy_5(in_ptr0, in_ptr1, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp56 = tl.load(in_ptr1 + (x1 + (8*tmp55)), rmask, eviction_policy='evict_last').to(tl.float32)
        tmp57 = tmp31 + tmp56
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp89 = tl.load(in_ptr1 + (x1 + (8*tmp88)), rmask, eviction_policy='evict_first').to(tl.float32)
        tmp90 = tmp64 + tmp89
