...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/bfloat16/de/cdeiu4b6ck6gkl33ablzs47jzni6jps7bhp4jatlwkfncdjqkbsu.py
...
def triton_red_fused__log_softmax_backward_data_add_nll_loss_backward_nll_loss_forward_2(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, out_ptr1, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp16 = tl.load(in_ptr4 + (r1 + (128112*x0)), rmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp17 = tl.load(in_ptr0 + (r1 + (128112*x0)), rmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp29 = tl.load(in_ptr5 + (r1 + (128112*x0)), rmask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        ...
        tmp35 = tmp16 + tmp34
        tl.store(out_ptr1 + (r1 + (128112*x0)), tmp35, rmask)
''')
