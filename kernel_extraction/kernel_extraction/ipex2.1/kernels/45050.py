...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/amp_fp16/y5/cy5jtnpphnx5tehwihh6ncwk5ozfcun4hmfh2jdzfnogkovllc45.py
# Source Nodes: [add_9, mul_6, softmax_1], Original ATen: [aten._softmax, aten.add, aten.mul]
# add_9 => add_59
# mul_6 => mul_103
# softmax_1 => amax_1, convert_element_type_159, convert_element_type_160, div_1, exp_1, sub_26, sum_2
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*fp16', 1: '*fp16', 2: '*fp16', 3: '*fp16', 4: 'i32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax_add_mul_36', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax_add_mul_36(in_ptr0, in_ptr1, in_ptr2, out_ptr3, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 65536
    rnumel = 144
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp8 = tmp6 < tmp7
        tmp9 = tmp8 & tmp5
        tmp10 = tl.load(in_ptr1 + ((23*((11 + (23*(x0 // 4)) + (r2 // 12)) // 24)) + (92*(x0 % 4)) + (368*x1) + ((11 + (23*(x0 // 4)) + (r2 // 12)) % 24)), rmask & tmp9, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp35 = tmp33 < tmp34
        tmp36 = tmp35 & tmp32
        tmp37 = tl.load(in_ptr1 + ((23*((11 + (23*(x0 // 4)) + (r2 // 12)) // 24)) + (92*(x0 % 4)) + (368*x1) + ((11 + (23*(x0 // 4)) + (r2 // 12)) % 24)), rmask & tmp36, eviction_policy='evict_last', other=0.0).to(tl.float32)
        ...
        tmp43 = tmp42 < tmp34
        tmp44 = tmp43 & tmp41
        tmp45 = tl.load(in_ptr2 + ((23*(((11 + (23*(x0 % 4)) + (r2 % 12)) // 24) % 4)) + (92*(x0 // 4)) + (368*x1) + ((11 + (23*(x0 % 4)) + (r2 % 12)) % 24)), rmask & tmp44, eviction_policy='evict_last', other=0.0).to(tl.float32)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp57 = 0.25
        tmp58 = tmp56 * tmp57
        tmp59 = 11 + (23*(x0 // 4)) + (r2 // 12)
        tmp60 = tl.full([1, 1], 96, tl.int64)
        tmp61 = tmp59 < tmp60
        tmp62 = (11 + (23*(x0 // 4)) + (r2 // 12)) % 24
        tmp63 = tl.full([1, 1], 23, tl.int64)
        tmp64 = tmp62 < tmp63
        tmp65 = tmp64 & tmp61
        tmp66 = tl.load(in_ptr1 + ((23*((11 + (23*(x0 // 4)) + (r2 // 12)) // 24)) + (92*(x0 % 4)) + (368*x1) + ((11 + (23*(x0 // 4)) + (r2 // 12)) % 24)), rmask & tmp65, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp67 = tl.where(tmp65, tmp66, 0.0)
        tmp68 = tl.where(tmp61, tmp67, 0.0)
        tmp69 = 11 + (23*(x0 % 4)) + (r2 % 12)
        tmp70 = tmp69 < tmp60
        tmp71 = (11 + (23*(x0 % 4)) + (r2 % 12)) % 24
        tmp72 = tmp71 < tmp63
        tmp73 = tmp72 & tmp70
        tmp74 = tl.load(in_ptr2 + ((23*(((11 + (23*(x0 % 4)) + (r2 % 12)) // 24) % 4)) + (92*(x0 // 4)) + (368*x1) + ((11 + (23*(x0 % 4)) + (r2 % 12)) % 24)), rmask & tmp73, eviction_policy='evict_first', other=0.0).to(tl.float32)
