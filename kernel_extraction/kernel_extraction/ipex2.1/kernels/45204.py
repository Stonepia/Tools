...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/float32/ix/cix3okanszlfluc6syt44fosicnstday57wbekftwh2n5acsa3dm.py
...
triton_red_fused__softmax_add_mul_34 = async_compile.triton('triton_red_fused__softmax_add_mul_34', '''
import triton
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*fp32', 1: '*fp32', 2: '*fp32', 3: '*fp32', 4: 'i32', 5: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax_add_mul_34', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax_add_mul_34(in_ptr0, in_ptr1, in_ptr2, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    xnumel = 65536
    rnumel = 144
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp8 = tmp6 < tmp7
        tmp9 = tmp8 & tmp5
        tmp10 = tl.load(in_ptr1 + ((23*((11 + (23*(x0 // 8)) + (r2 // 12)) // 24)) + (184*(x0 % 8)) + (1472*((((8*x1) + (x0 % 8)) // 8) % 1024)) + ((11 + (23*(x0 // 8)) + (r2 // 12)) % 24)), rmask & tmp9, eviction_policy='evict_last', other=0.0)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp34 = tmp32 < tmp33
        tmp35 = tmp34 & tmp31
        tmp36 = tl.load(in_ptr1 + ((23*((11 + (23*(x0 // 8)) + (r2 // 12)) // 24)) + (184*(x0 % 8)) + (1472*((((8*x1) + (x0 % 8)) // 8) % 1024)) + ((11 + (23*(x0 // 8)) + (r2 // 12)) % 24)), rmask & tmp35, eviction_policy='evict_last', other=0.0)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        tmp62 = tmp60 < tmp61
        tmp63 = tmp62 & tmp59
        tmp64 = tl.load(in_ptr1 + ((23*((11 + (23*(x0 // 8)) + (r2 // 12)) // 24)) + (184*(x0 % 8)) + (1472*((((8*x1) + (x0 % 8)) // 8) % 1024)) + ((11 + (23*(x0 // 8)) + (r2 // 12)) % 24)), rmask & tmp63, eviction_policy='evict_first', other=0.0)
