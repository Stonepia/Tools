...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/huggingface/amp_bf16/on/con6p4pke2dgz3wnwa2v7jhuxxmeuvxz7mhitovxj6jj5pr2srd4.py
...
@reduction(
    ...
    filename=__file__,
    meta={'signature': {0: '*bf16', 1: '*fp32', 2: '*i64', 3: '*fp32', 4: '*fp32', 5: '*i1', 6: '*fp32', 7: '*bf16', 8: 'i32', 9: 'i32', 10: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_red_fused__softmax__to_copy_native_dropout_5', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2, 3, 4, 5, 6, 7, 9, 10), equal_to_1=())]}
)
@triton.jit
def triton_red_fused__softmax__to_copy_native_dropout_5(in_ptr0, in_ptr1, in_ptr2, out_ptr1, out_ptr3, out_ptr4, out_ptr5, out_ptr6, load_seed_offset, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r3 = rindex
        tmp75 = tl.load(out_ptr3 + (r3 + (1024*x4)), rmask, eviction_policy='evict_first', other=0.0)
        tmp79 = tl.load(out_ptr1 + (r3 + (1024*x4)), rmask, eviction_policy='evict_first', other=0.0)
        ...
        tmp86 = tmp84 * tmp85
        tl.store(out_ptr4 + (r3 + (1024*x4)), tmp78, rmask)
        tl.store(out_ptr5 + (r3 + (1024*x4)), tmp81, rmask)
        tl.store(out_ptr6 + (r3 + (1024*x4)), tmp86, rmask)
''')
