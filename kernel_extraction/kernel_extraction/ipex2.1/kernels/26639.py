...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/torchbench/float32/77/c77lywpab2njpq6jqtdz72hh34r2e3brocav5zzznnbhgjxqdohc.py
...
def triton_poi_fused_add_convolution_11(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, out_ptr0, ynumel, xnumel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    ...
    tmp12 = tl.load(in_ptr4 + (x2 + (256*y1)), xmask, eviction_policy='evict_last')
    tmp14 = tl.load(in_ptr5 + (x2 + (256*y1)), xmask, eviction_policy='evict_last')
    tmp3 = tmp1 - tmp2
    tmp5 = 1e-05
    tmp6 = tmp4 + tmp5
    tmp7 = tl.sqrt(tmp6)
    ...
    tmp15 = tmp13 + tmp14
    tmp16 = tmp0 + tmp15
    tl.store(out_ptr0 + (y0 + (1024*x2) + (262144*y1)), tmp16, xmask)
''')
