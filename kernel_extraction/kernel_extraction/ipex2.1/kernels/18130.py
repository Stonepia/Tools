...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/float32/x4/cx44d4oqizb2eikkppq5f7wj7273pgscjpinfxp54ywwoi7hfa3u.py
...
@pointwise(size_hints=[4194304], filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_constant_pad_nd_max_pool2d_with_indices_38', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2), equal_to_1=())]})
@triton.jit
def triton_poi_fused_constant_pad_nd_max_pool2d_with_indices_38(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp4 = tmp3 < tmp1
    tmp5 = tmp2 & tmp4
    tmp6 = tl.load(in_ptr0 + (x0 + (864*x1) + (36288*x5)), tmp5 & xmask, other=0.0)
    tmp7 = tl.where(tmp5, tmp6, float("-inf"))
    tmp8 = 1 + (2*x1)
    tmp9 = tmp8 < tmp1
    tmp10 = tmp2 & tmp9
    tmp11 = tl.load(in_ptr0 + (432 + x0 + (864*x1) + (36288*x5)), tmp10 & xmask, other=0.0)
    tmp12 = tl.where(tmp10, tmp11, float("-inf"))
    tmp13 = triton_helpers.maximum(tmp12, tmp7)
    tmp14 = 2 + (2*x1)
    tmp15 = tmp14 < tmp1
    tmp16 = tmp2 & tmp15
    tmp17 = tl.load(in_ptr0 + (864 + x0 + (864*x1) + (36288*x5)), tmp16 & xmask, other=0.0)
    tmp18 = tl.where(tmp16, tmp17, float("-inf"))
    tmp19 = triton_helpers.maximum(tmp18, tmp13)
    tmp20 = 1 + (2*x2)
    tmp21 = tmp20 < tmp1
    tmp22 = tmp21 & tmp4
    tmp23 = tl.load(in_ptr0 + (18144 + x0 + (864*x1) + (36288*x5)), tmp22 & xmask, other=0.0)
    tmp24 = tl.where(tmp22, tmp23, float("-inf"))
    tmp25 = triton_helpers.maximum(tmp24, tmp19)
    tmp26 = tmp21 & tmp9
    tmp27 = tl.load(in_ptr0 + (18576 + x0 + (864*x1) + (36288*x5)), tmp26 & xmask, other=0.0)
    tmp28 = tl.where(tmp26, tmp27, float("-inf"))
    tmp29 = triton_helpers.maximum(tmp28, tmp25)
    tmp30 = tmp21 & tmp15
    tmp31 = tl.load(in_ptr0 + (19008 + x0 + (864*x1) + (36288*x5)), tmp30 & xmask, other=0.0)
    tmp32 = tl.where(tmp30, tmp31, float("-inf"))
    tmp33 = triton_helpers.maximum(tmp32, tmp29)
    tmp34 = 2 + (2*x2)
    tmp35 = tmp34 < tmp1
    tmp36 = tmp35 & tmp4
    tmp37 = tl.load(in_ptr0 + (36288 + x0 + (864*x1) + (36288*x5)), tmp36 & xmask, other=0.0)
    tmp38 = tl.where(tmp36, tmp37, float("-inf"))
    tmp39 = triton_helpers.maximum(tmp38, tmp33)
    tmp40 = tmp35 & tmp9
    tmp41 = tl.load(in_ptr0 + (36720 + x0 + (864*x1) + (36288*x5)), tmp40 & xmask, other=0.0)
    tmp42 = tl.where(tmp40, tmp41, float("-inf"))
    tmp43 = triton_helpers.maximum(tmp42, tmp39)
    tmp44 = tmp35 & tmp15
    tmp45 = tl.load(in_ptr0 + (37152 + x0 + (864*x1) + (36288*x5)), tmp44 & xmask, other=0.0)
