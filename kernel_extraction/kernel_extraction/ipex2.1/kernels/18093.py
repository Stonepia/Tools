...
# kernel path: /home/sdp/tongsu/pytorch/inductor_log/timm_models/float32/on/con6wzvzxcfdvixmlrlsfnoukslhlnz32by3xmwxo7hvejdmkue4.py
...
@pointwise(size_hints=[16777216], filename=__file__, meta={'signature': {0: '*fp32', 1: '*fp32', 2: 'i32'}, 'device': 0, 'device_type': 'xpu', 'constants': {}, 'mutated_arg_names': [], 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_constant_pad_nd_max_pool2d_with_indices_1', 'configs': [AttrsDescriptor(divisible_by_16=(0, 1, 2), equal_to_1=())]})
@triton.jit
def triton_poi_fused_constant_pad_nd_max_pool2d_with_indices_1(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    ...
    tmp9 = tmp8 & tmp6
    tmp10 = tmp9 & tmp7
    tmp11 = tl.load(in_ptr0 + ((-15936) + x0 + (192*x1) + (31680*x2) + (2613600*x3)), tmp10 & xmask, other=0.0)
    ...
    tmp16 = tmp8 & tmp14
    tmp17 = tmp16 & tmp15
    tmp18 = tl.load(in_ptr0 + ((-15840) + x0 + (192*x1) + (31680*x2) + (2613600*x3)), tmp17 & xmask, other=0.0)
    ...
    tmp24 = tmp8 & tmp22
    tmp25 = tmp24 & tmp23
    tmp26 = tl.load(in_ptr0 + ((-15744) + x0 + (192*x1) + (31680*x2) + (2613600*x3)), tmp25 & xmask, other=0.0)
    ...
    tmp33 = tmp32 & tmp6
    tmp34 = tmp33 & tmp7
    tmp35 = tl.load(in_ptr0 + ((-96) + x0 + (192*x1) + (31680*x2) + (2613600*x3)), tmp34 & xmask, other=0.0)
    tmp36 = tl.where(tmp34, tmp35, float("-inf"))
    tmp37 = triton_helpers.maximum(tmp36, tmp28)
    tmp38 = tmp32 & tmp14
    tmp39 = tmp38 & tmp15
    tmp40 = tl.load(in_ptr0 + (x0 + (192*x1) + (31680*x2) + (2613600*x3)), tmp39 & xmask, other=0.0)
    tmp41 = tl.where(tmp39, tmp40, float("-inf"))
    tmp42 = triton_helpers.maximum(tmp41, tmp37)
    tmp43 = tmp32 & tmp22
    tmp44 = tmp43 & tmp23
    tmp45 = tl.load(in_ptr0 + (96 + x0 + (192*x1) + (31680*x2) + (2613600*x3)), tmp44 & xmask, other=0.0)
    ...
    tmp52 = tmp51 & tmp6
    tmp53 = tmp52 & tmp7
    tmp54 = tl.load(in_ptr0 + (15744 + x0 + (192*x1) + (31680*x2) + (2613600*x3)), tmp53 & xmask, other=0.0)
    tmp55 = tl.where(tmp53, tmp54, float("-inf"))
    tmp56 = triton_helpers.maximum(tmp55, tmp47)
    tmp57 = tmp51 & tmp14
    tmp58 = tmp57 & tmp15
    tmp59 = tl.load(in_ptr0 + (15840 + x0 + (192*x1) + (31680*x2) + (2613600*x3)), tmp58 & xmask, other=0.0)
    tmp60 = tl.where(tmp58, tmp59, float("-inf"))
    tmp61 = triton_helpers.maximum(tmp60, tmp56)
    tmp62 = tmp51 & tmp22
    tmp63 = tmp62 & tmp23
    tmp64 = tl.load(in_ptr0 + (15936 + x0 + (192*x1) + (31680*x2) + (2613600*x3)), tmp63 & xmask, other=0.0)
